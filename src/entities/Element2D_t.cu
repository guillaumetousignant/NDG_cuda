...
template<> __device__ auto SEM::Device::Entities::Element2D_t::estimate_error<SEM::Device::Polynomials::ChebyshevPolynomial_t>(deviceFloat tolerance_min, deviceFloat tolerance_max, const deviceFloat* polynomial_nodes, const deviceFloat* weights) -> void;
template<> __device__ auto SEM::Device::Entities::Element2D_t::estimate_error<SEM::Device::Polynomials::LegendrePolynomial_t>(deviceFloat tolerance_min, deviceFloat tolerance_max, const deviceFloat* polynomial_nodes, const deviceFloat* weights) -> void;

template<typename Polynomial>
__device__
auto SEM::Device::Entities::Element2D_t::estimate_error(deviceFloat tolerance_min, deviceFloat tolerance_max, const deviceFloat* polynomial_nodes, const deviceFloat* weights) -> void {
    const int offset_1D = N_ * (N_ + 1) /2;
    const int n_points_least_squares = min(N_ + 1, SEM::Device::Constants::n_points_least_squares_max); // Number of points to use for thew least squares reduction, but don't go above N.
