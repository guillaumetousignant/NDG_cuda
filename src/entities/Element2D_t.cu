...
auto SEM::Device::Entities::Element2D_t::interpolate_to_boundaries(const deviceFloat* lagrange_interpolant_minus, const deviceFloat* lagrange_interpolant_plus) -> void {
    ...
    for (int i = 0; i <= N_; ++i) {
        p_extrapolated_[0][i] = deviceFloat{0};
        p_extrapolated_[2][N_ - i] = deviceFloat{0};
        p_extrapolated_[1][i] = deviceFloat{0};
        p_extrapolated_[3][N_ - i] = deviceFloat{0};

        u_extrapolated_[0][i] = deviceFloat{0};
        u_extrapolated_[2][N_ - i] = deviceFloat{0};
        u_extrapolated_[1][i] = deviceFloat{0};
        u_extrapolated_[3][N_ - i] = deviceFloat{0};

        v_extrapolated_[0][i] = deviceFloat{0};
        v_extrapolated_[2][N_ - i] = deviceFloat{0};
        v_extrapolated_[1][i] = deviceFloat{0};
        v_extrapolated_[3][N_ - i] = deviceFloat{0};
...
__device__
auto SEM::Device::Entities::Element2D_t::estimate_error(deviceFloat tolerance_min, deviceFloat tolerance_max, const deviceFloat* polynomials) -> void {
    ...
    for (int node_index = 0; node_index < n_points_least_squares; ++node_index) {
        ...
        // x and y directions
        for (int i = 0; i < p; ++i) {
            deviceFloat local_spectrum_x{0};
            deviceFloat local_spectrum_y{0};

            for (int k = 0; k <= N_; ++k) {
                for (int l = 0; l <= N_; ++l) {
                    local_spectrum_x += p_[k * (N_ + 1) + l] * polynomials[offset_2D + i * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
                    local_spectrum_y += p_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + i * (N_ + 1) + l];
                }
            }
        ...
        for (int k = 0; k <= N_; ++k) {
            for (int l = 0; l <= N_; ++l) {
                local_spectrum_x += p_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
            }
    ...
    for (int node_index = 0; node_index < n_points_least_squares; ++node_index) {
        ...
        for (int i = 0; i < p; ++i) {
            ...
            for (int k = 0; k <= N_; ++k) {
                for (int l = 0; l <= N_; ++l) {
                    local_spectrum_x += u_[k * (N_ + 1) + l] * polynomials[offset_2D + i * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
                    local_spectrum_y += u_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + i * (N_ + 1) + l];
                }
            }
            spectrum_[node_index] += std::abs(local_spectrum_x) + std::abs(local_spectrum_y);
        }
        ...
        for (int k = 0; k <= N_; ++k) {
            for (int l = 0; l <= N_; ++l) {
                local_spectrum_x += u_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
            }
        }
        spectrum_[node_index] += std::abs(local_spectrum_x);
    }
    ...
    for (int node_index = 0; node_index < n_points_least_squares; ++node_index) {
        ...
        for (int i = 0; i < p; ++i) {
            ...
            for (int k = 0; k <= N_; ++k) {
                for (int l = 0; l <= N_; ++l) {
                    local_spectrum_x += v_[k * (N_ + 1) + l] * polynomials[offset_2D + i * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
                    local_spectrum_y += v_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + i * (N_ + 1) + l];
                }
            }
            spectrum_[node_index] += std::abs(local_spectrum_x) + std::abs(local_spectrum_x);
        }
        ...
        for (int k = 0; k <= N_; ++k) {
            for (int l = 0; l <= N_; ++l) {
                local_spectrum_x += v_[k * (N_ + 1) + l] * polynomials[offset_2D + p * (N_ + 1) + k] * polynomials[offset_2D + p * (N_ + 1) + l];
            }
...
__device__
auto SEM::Device::Entities::Element2D_t::estimate_p_error(deviceFloat tolerance_min, deviceFloat tolerance_max, const deviceFloat* polynomials) -> void {
    const size_t offset_2D = N_ * (N_ + 1) * (2 * N_ + 1) /6;
    const int n_points_least_squares = std::min(N_ + 1, SEM::Device::Constants::n_points_least_squares_max); // Number of points to use for thew least squares reduction, but don't go above N.
    ...
    for (int i = 0; i <= N_; ++i) {
        for (int j = 0; j <= N_; ++j) {
            ...
            // A row fits exactly
                ...
                deviceFloat p_numerator{0};
                deviceFloat u_numerator{0};
                deviceFloat v_numerator{0};
                deviceFloat G_p_numerator{0};
                deviceFloat G_u_numerator{0};
                deviceFloat G_v_numerator{0};
                deviceFloat denominator{0};
            ...
            // Complete interpolation
                ...
                p_[i * (N_ + 1) + j] = deviceFloat{0};
                u_[i * (N_ + 1) + j] = deviceFloat{0};
                v_[i * (N_ + 1) + j] = deviceFloat{0};
                G_p_[i * (N_ + 1) + j] = deviceFloat{0};
                G_u_[i * (N_ + 1) + j] = deviceFloat{0};
                G_v_[i * (N_ + 1) + j] = deviceFloat{0};

                deviceFloat denominator_x{0};
                deviceFloat denominator_y{0};
...
auto SEM::Device::Entities::Element2D_t::interpolate_solution(size_t n_interpolation_points, const std::array<Vec2<deviceFloat>, 4>& points, const deviceFloat* interpolation_matrices, deviceFloat* x, deviceFloat* y, deviceFloat* p, deviceFloat* u, deviceFloat* v) const -> void {
    for (size_t i = 0; i < n_interpolation_points; ++i) {
        for (size_t j = 0; j < n_interpolation_points; ++j) {
            ...
            // Pressure, u, and v
            p[i * n_interpolation_points + j] = deviceFloat{0};
            u[i * n_interpolation_points + j] = deviceFloat{0};
            v[i * n_interpolation_points + j] = deviceFloat{0};
            for (int m = 0; m <= N_; ++m) {
                for (int n = 0; n <= N_; ++n) {
...
auto SEM::Device::Entities::Element2D_t::interpolate_complete_solution(size_t n_interpolation_points, deviceFloat time, const std::array<Vec2<deviceFloat>, 4>& points, const deviceFloat* polynomial_nodes, const deviceFloat* interpolation_matrices, deviceFloat* x, deviceFloat* y, deviceFloat* p, deviceFloat* u, deviceFloat* v, deviceFloat* dp_dt, deviceFloat* du_dt, deviceFloat* dv_dt, deviceFloat* p_analytical_error, deviceFloat* u_analytical_error, deviceFloat* v_analytical_error) const -> void {
    ...
    for (size_t i = 0; i < n_interpolation_points; ++i) {
        for (size_t j = 0; j < n_interpolation_points; ++j) {
            ...
            u[i * n_interpolation_points + j] = deviceFloat{0};
            v[i * n_interpolation_points + j] = deviceFloat{0};
            dp_dt[i * n_interpolation_points + j] = deviceFloat{0};
            du_dt[i * n_interpolation_points + j] = deviceFloat{0};
            dv_dt[i * n_interpolation_points + j] = deviceFloat{0};
            p_analytical_error[i * n_interpolation_points + j] = deviceFloat{0};
            u_analytical_error[i * n_interpolation_points + j] = deviceFloat{0};
            v_analytical_error[i * n_interpolation_points + j] = deviceFloat{0};
            for (int m = 0; m <= N_; ++m) {
                for (int n = 0; n <= N_; ++n) {
                    p[i * n_interpolation_points + j] += p_[m * (N_ + 1) + n] * interpolation_matrices[i * (N_ + 1) + m] * interpolation_matrices[j * (N_ + 1) + n];
                    u[i * n_interpolation_points + j] += u_[m * (N_ + 1) + n] * interpolation_matrices[i * (N_ + 1) + m] * interpolation_matrices[j * (N_ + 1) + n];
...
auto SEM::Device::Entities::Element2D_t::compute_geometry(const std::array<Vec2<deviceFloat>, 4>& points, const deviceFloat* polynomial_nodes) -> void {
    ...
    for (int i = 0; i <= N_; ++i) {
        ...
        const Vec2<deviceFloat> coordinates_bottom {polynomial_nodes[offset_1D + i], deviceFloat{-1}};
        const Vec2<deviceFloat> coordinates_right  {deviceFloat{1}, polynomial_nodes[offset_1D + i]};
        const Vec2<deviceFloat> coordinates_top    {polynomial_nodes[offset_1D + i], deviceFloat{1}};
        const Vec2<deviceFloat> coordinates_left   {deviceFloat{-1}, polynomial_nodes[offset_1D + i]};
