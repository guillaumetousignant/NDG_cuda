...
auto SEM::Device::Meshes::Mesh2D_t::estimate_error(const device_vector<deviceFloat>& polynomials) -> void {
    SEM::Device::Meshes::estimate_error<<<elements_numBlocks_, elements_blockSize_, 0, stream_>>>(n_elements_, elements_.data(), tolerance_min_, tolerance_max_, polynomials.data());
}
                ...
                if (element.additional_nodes_[side_index]) {
                    ...
                    for (size_t side_face_index = 0; side_face_index < element.faces_[side_index].size(); ++side_face_index) {
                        ...
                        if (face.refine_) {
                            ...
                            if (C_proj + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                                && C_proj <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                                && D_proj + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                                && D_proj <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
        
                                ++n_side_faces;
                            }
                            // The second half of the face is within the element
                            if (D_proj + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                                && D_proj <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                                && E_proj + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                                && E_proj <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
    ...
    for (size_t face_index = index; face_index < n_faces; face_index += stride) {
        ...
        if (face.refine_) {
            ...
            if (element_L.would_h_refine(max_split_level)) {
                ...
                if (C_proj[0] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && C_proj[0] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                    && D_proj[0] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && D_proj[0] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
                ...
                if (D_proj[0] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && D_proj[0] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                    && E_proj[0] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && E_proj[0] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
                ...
                if (C_proj[1] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && C_proj[1] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                    && D_proj[1] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && D_proj[1] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
                ...
                if (D_proj[1] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && D_proj[1] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()
                    && E_proj[1] + std::numeric_limits<deviceFloat>::epsilon() >= deviceFloat{0} 
                    && E_proj[1] <= deviceFloat{1} + std::numeric_limits<deviceFloat>::epsilon()) {
        ...
        if (elements_splitting[i]) {            
            ...
            if (n_side_faces[0] == 0 || n_side_faces[1] == 0) {
                ...
                for (size_t side_face_index = 0; side_face_index < element.faces_[0].size(); ++side_face_index) {
                    ...
                    Vec2<deviceFloat> other_element_centre {0};
                    ...
                    const std::array<Vec2<deviceFloat>, 2> this_element_nodes = (n_side_faces[0] == 0) ? std::array<Vec2<deviceFloat>, 2>{new_node, side_nodes[1]} : std::array<Vec2<deviceFloat>, 2>{side_nodes[0], new_node};
                    const std::array<Vec2<deviceFloat>, 2> other_element_nodes {nodes[other_element.nodes_[other_element_side_index]], nodes[other_element.nodes_[other_element_next_side_index]]};

                    const Vec2<deviceFloat> this_element_centre = (this_element_nodes[0] + this_element_nodes[1])/2;
                    const std::array<Vec2<deviceFloat>, 2> elements_centres = (other_element_face_side_index == 0) ? std::array<Vec2<deviceFloat>, 2>{other_element_centre, this_element_centre} : std::array<Vec2<deviceFloat>, 2>{this_element_centre, other_element_centre};
                    
                    std::array<std::array<Vec2<deviceFloat>, 2>, 2> element_geometry_nodes = (other_element_face_side_index == 0) ? std::array<std::array<Vec2<deviceFloat>, 2>, 2>{other_element_nodes, this_element_nodes} : std::array<std::array<Vec2<deviceFloat>, 2>, 2>{this_element_nodes, other_element_nodes};
        ...
        for (size_t j = 0; j < element.faces_.size(); ++j) {
            ...
            for (size_t k = 0; k < side_n_neighbours; ++k) {
                ...
                if (neighbour_element_index >= n_domain_elements 
                    ...
                    // We create
                    const size_t next_side = (j + 1 < element.nodes_.size()) ? j + 1 : size_t{0};
                    const size_t neighbour_next_side = (neighbour_side + 1 < neighbour_element.nodes_.size()) ? neighbour_side + 1 : size_t{0};
