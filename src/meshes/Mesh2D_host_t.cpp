...
SEM::Meshes::Mesh2D_host_t::Mesh2D_host_t(std::filesystem::path filename, int initial_N, int maximum_N, size_t n_interpolation_points, int max_split_level, int adaptivity_interval, int load_balancing_interval, hostFloat tolerance_min, hostFloat tolerance_max, const std::vector<hostFloat>& polynomial_nodes) :       
    ...
    compute_element_geometry(n_elements_, elements_.data(), nodes_.data(), polynomial_nodes.data());
    compute_boundary_geometry(n_elements_, elements_.size(), elements_.data(), nodes_.data(), polynomial_nodes.data());
...
auto SEM::Meshes::Mesh2D_host_t::read_su2(std::filesystem::path filename) -> void {
    std::cerr << "Error: SU2 meshes not implemented yet. Exiting." << std::endl;
    exit(15);
}

auto SEM::Meshes::Mesh2D_host_t::read_cgns(std::filesystem::path filename) -> void {
    int index_file = 0;
    ...
    elements_ = std::vector<Element2D_host_t>(n_elements);
    ...
    for (int i = 0; i < n_sections; ++i) {
        if (section_is_domain[i]) {
            ...
            for (cgsize_t j = 0; j < section_ranges[i][1] - section_ranges[i][0] + 1; ++j) {
                Element2D_host_t& element = elements_[section_start_indices[i] + j];
    ...
    if (n_mpi_interface_elements > 0) {
        ...
        device_interfaces_p_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        device_interfaces_u_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        device_interfaces_v_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        device_receiving_interfaces_p_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
        device_receiving_interfaces_u_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
        device_receiving_interfaces_v_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
        interfaces_N_ = std::vector<int>(mpi_interfaces_origin_.size());
        host_interfaces_p_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        host_interfaces_u_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        host_interfaces_v_ = std::vector<hostFloat>(mpi_interfaces_origin_.size() * (maximum_N_ + 1));
        interfaces_refine_ = std::vector<unsigned int>(mpi_interfaces_origin_.size());
        host_receiving_interfaces_p_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
        host_receiving_interfaces_u_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
        host_receiving_interfaces_v_ = std::vector<hostFloat>(mpi_interfaces_destination.size() * (maximum_N_ + 1));
...
auto SEM::Meshes::Mesh2D_host_t::build_node_to_element(size_t n_nodes, const std::vector<Element2D_host_t>& elements) -> std::vector<std::vector<size_t>> {
...
auto SEM::Meshes::Mesh2D_host_t::build_element_to_element(const std::vector<Element2D_host_t>& elements, const std::vector<std::vector<size_t>>& node_to_element) -> std::vector<std::vector<size_t>> {
    std::vector<std::vector<size_t>> element_to_element(elements.size());

    for (size_t i = 0; i < elements.size(); ++i) {
        const Element2D_host_t& element = elements[i];
        ...
        for (size_t j = 0; j < element.nodes_.size(); ++j) {
            ...
            for (auto element_index : node_to_element[node_index]) {
                if (element_index != i) {
                    const Element2D_host_t& element_neighbor = elements[element_index];
...
auto SEM::Meshes::Mesh2D_host_t::build_faces(size_t n_elements_domain, size_t n_nodes, int initial_N, const std::vector<Element2D_host_t>& elements) -> std::tuple<std::vector<Face2D_host_t>, std::vector<std::vector<size_t>>, std::vector<std::array<size_t, 4>>> {
    size_t total_edges = 0;
    for (const auto& element: elements) {
        total_edges += element.nodes_.size();
    }

    std::vector<Face2D_host_t> faces;
...
auto SEM::Meshes::Mesh2D_host_t::initial_conditions(const std::vector<hostFloat>& polynomial_nodes) -> void {
    initial_conditions_2D(n_elements_, elements_.data(), nodes_.data(), polynomial_nodes.data());
}

auto SEM::Meshes::print_element_faces(size_t n_elements, const Element2D_host_t* elements) -> void {
    for (size_t element_index = 0; element_index < n_elements; ++element_index) {
        const Element2D_host_t& element = elements[element_index];
...
auto SEM::Meshes::print_boundary_element_faces(size_t n_domain_elements, size_t n_total_elements, const Element2D_host_t* elements) -> void {
...
auto SEM::Meshes::Mesh2D_host_t::print() const -> void {
    ...
    SEM::Meshes::print_element_faces(n_elements_, elements_.data());
    SEM::Meshes::print_boundary_element_faces(n_elements_, elements_.size(), elements_.data());

    std::cout << std::endl;
}

auto SEM::Meshes::Mesh2D_host_t::write_data(hostFloat time, const std::vector<hostFloat>& interpolation_matrices, const SEM::Helpers::DataWriter_t& data_writer) -> void {
    SEM::Meshes::get_solution(n_elements_, n_interpolation_points_, elements_.data(), nodes_.data(), interpolation_matrices.data(), x_output_.data(), y_output_.data(), p_output_.data(), u_output_.data(), v_output_.data());

    data_writer.write_data(n_interpolation_points_, n_elements_, time, x_output_, y_output_, p_output_, u_output_, v_output_);
}

auto SEM::Meshes::Mesh2D_host_t::write_complete_data(hostFloat time, const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& interpolation_matrices, const SEM::Helpers::DataWriter_t& data_writer) -> void {
    ...
    SEM::Meshes::get_complete_solution(n_elements_, n_interpolation_points_, time, elements_.data(), nodes_.data(), polynomial_nodes.data(), interpolation_matrices.data(), x_output_.data(), y_output_.data(), p_output_.data(), u_output_.data(), v_output_.data(), N.data(), dp_dt.data(), du_dt.data(), dv_dt.data(), p_error.data(), u_error.data(), v_error.data(), p_sigma.data(), u_sigma.data(), v_sigma.data(), refine.data(), coarsen.data(), split_level.data(), p_analytical_error.data(), u_analytical_error.data(), v_analytical_error.data(), status.data(), rotation.data());
...
auto SEM::Meshes::Mesh2D_host_t::adapt(int N_max, const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& barycentric_weights) -> void {
    ...
    if (n_splitting_elements == 0) {
        if (!mpi_interfaces_origin_.empty()) {
            ...
            SEM::Meshes::get_MPI_interfaces_N(mpi_interfaces_origin_.size(), N_max, elements_.data(), mpi_interfaces_origin_.data(), interfaces_N_.data());
            ...
            SEM::Meshes::copy_mpi_interfaces_error(mpi_interfaces_destination_.size(), elements_.data(), faces_.data(), nodes_.data(), mpi_interfaces_destination_.data(), receiving_interfaces_N_.data(), receiving_interfaces_refine_.data());
        ...
        if (n_splitting_mpi_interface_incoming_elements == 0) {
            SEM::Meshes::p_adapt(n_elements_, elements_.data(), N_max, nodes_.data(), polynomial_nodes.data(), barycentric_weights.data());

            // We need to adjust the boundaries in all cases, or check if of our neighbours have to change
            if (!wall_boundaries_.empty()) {
                SEM::Meshes::adjust_boundaries(wall_boundaries_.size(), elements_.data(), wall_boundaries_.data(), faces_.data());
            }
            if (!symmetry_boundaries_.empty()) {
                SEM::Meshes::adjust_boundaries(symmetry_boundaries_.size(), elements_.data(), symmetry_boundaries_.data(), faces_.data());
            }
            if (!inflow_boundaries_.empty()) {
                SEM::Meshes::adjust_boundaries(inflow_boundaries_.size(), elements_.data(), inflow_boundaries_.data(), faces_.data());
            }
            if (!outflow_boundaries_.empty()) {
                SEM::Meshes::adjust_boundaries(outflow_boundaries_.size(), elements_.data(), outflow_boundaries_.data(), faces_.data());
            }
            if (!interfaces_origin_.empty()) {
                SEM::Meshes::adjust_interfaces(interfaces_origin_.size(), elements_.data(), interfaces_origin_.data(), interfaces_destination_.data());
            }
            if (!mpi_interfaces_destination_.empty()) {
                SEM::Meshes::adjust_MPI_incoming_interfaces(mpi_interfaces_destination_.size(), elements_.data(), mpi_interfaces_destination_.data(), receiving_interfaces_N_.data(), nodes_.data(), polynomial_nodes.data());
            }

            SEM::Meshes::adjust_faces(faces_.size(), faces_.data(), elements_.data());
        }
        else {
            SEM::Meshes::no_new_nodes(n_elements_, elements_.data());
            ...
            std::vector<Element2D_host_t> new_elements(elements_.size() + n_splitting_mpi_interface_incoming_elements);
            ...
            if (n_splitting_faces == 0) {
                SEM::Meshes::p_adapt_move(n_elements_, elements_.data(), new_elements.data(), N_max, nodes_.data(), polynomial_nodes.data(), barycentric_weights.data(), elements_new_indices.data());

                if (!wall_boundaries_.empty()) {
                    SEM::Meshes::move_boundaries(wall_boundaries_.size(), elements_.data(), new_elements.data(), wall_boundaries_.data(), faces_.data(), elements_new_indices.data());
                }
                if (!symmetry_boundaries_.empty()) {
                    SEM::Meshes::move_boundaries(symmetry_boundaries_.size(), elements_.data(), new_elements.data(), symmetry_boundaries_.data(), faces_.data(), elements_new_indices.data());
                }
                if (!inflow_boundaries_.empty()) {
                    SEM::Meshes::move_boundaries(inflow_boundaries_.size(), elements_.data(), new_elements.data(), inflow_boundaries_.data(), faces_.data(), elements_new_indices.data());
                }
                if (!outflow_boundaries_.empty()) {
                    SEM::Meshes::move_boundaries(outflow_boundaries_.size(), elements_.data(), new_elements.data(), outflow_boundaries_.data(), faces_.data(), elements_new_indices.data());
                }
                if (!interfaces_origin_.empty()) {
                    SEM::Meshes::move_interfaces(interfaces_origin_.size(), elements_.data(), new_elements.data(), interfaces_origin_.data(), interfaces_destination_.data(), elements_new_indices.data());
                }

                // MPI
                SEM::Meshes::split_mpi_incoming_interfaces(mpi_interfaces_destination_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + wall_boundaries_.size() + symmetry_boundaries_.size() + inflow_boundaries_.size() + outflow_boundaries_.size() + interfaces_origin_.size(), elements_.data(), new_elements.data(), mpi_interfaces_destination_.data(), new_mpi_interfaces_destination.data(), faces_.data(), nodes_.data(), device_faces_refine_array_.data(), device_mpi_interfaces_incoming_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, receiving_interfaces_N_.data(), receiving_interfaces_refine_.data(), elements_new_indices.data());

                // Faces
                SEM::Meshes::adjust_faces_neighbours(faces_.size(), faces_.data(), elements_.data(), nodes_.data(), max_split_level_, N_max, elements_new_indices.data());
            }
            else {
                std::vector<Vec2<hostFloat>> new_nodes(nodes_.size() + n_splitting_faces);
                std::vector<Face2D_host_t> new_faces(faces_.size() + n_splitting_faces);

                for (size_t i = 0; i < nodes_.size(); ++i) {
                    new_nodes[i] = nodes_[i];
                }
                SEM::Meshes::p_adapt_split_faces(n_elements_, faces_.size(), nodes_.size(), n_splitting_elements, elements_.data(), new_elements.data(), faces_.data(), N_max, new_nodes.data(), polynomial_nodes.data(), barycentric_weights.data(), device_faces_refine_array_.data(), faces_blockSize_, elements_new_indices.data());
                ...
                // MPI
                SEM::Meshes::split_mpi_incoming_interfaces(mpi_interfaces_destination_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + wall_boundaries_.size() + symmetry_boundaries_.size() + inflow_boundaries_.size() + outflow_boundaries_.size() + interfaces_origin_.size(), elements_.data(), new_elements.data(), mpi_interfaces_destination_.data(), new_mpi_interfaces_destination.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_mpi_interfaces_incoming_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, receiving_interfaces_N_.data(), receiving_interfaces_refine_.data(), elements_new_indices.data());

                SEM::Meshes::split_faces(faces_.size(), nodes_.size(), n_splitting_elements, faces_.data(), new_faces.data(), elements_.data(), new_nodes.data(), device_faces_refine_array_.data(), max_split_level_, N_max, elements_new_indices.data());
            ...
            std::vector<hostFloat> new_device_receiving_interfaces_p(mpi_interfaces_destination_.size() * (maximum_N_ + 1), stream_);
            std::vector<hostFloat> new_device_receiving_interfaces_u(mpi_interfaces_destination_.size() * (maximum_N_ + 1), stream_);
            std::vector<hostFloat> new_device_receiving_interfaces_v(mpi_interfaces_destination_.size() * (maximum_N_ + 1), stream_);
            ...
            host_receiving_interfaces_p_ = std::vector<hostFloat>(mpi_interfaces_destination_.size() * (maximum_N_ + 1));
            host_receiving_interfaces_u_ = std::vector<hostFloat>(mpi_interfaces_destination_.size() * (maximum_N_ + 1));
            host_receiving_interfaces_v_ = std::vector<hostFloat>(mpi_interfaces_destination_.size() * (maximum_N_ + 1));
    ...
    if (!mpi_interfaces_origin_.empty()) {
        ...
        SEM::Meshes::get_MPI_interfaces_adaptivity(mpi_interfaces_origin_.size(), elements_.data(), mpi_interfaces_origin_.data(), interfaces_N_.data(), interfaces_refine_.data(), max_split_level_, N_max);
    ...
    SEM::Meshes::find_nodes(n_elements_, elements_.data(), faces_.data(), nodes_.data(), max_split_level_);
    if (!interfaces_origin_.empty()) {
        SEM::Meshes::copy_interfaces_error(interfaces_origin_.size(), elements_.data(), interfaces_origin_.data(), interfaces_origin_side_.data(), interfaces_destination_.data());
    }
    if (!wall_boundaries_.empty()) {
        SEM::Meshes::copy_boundaries_error(wall_boundaries_.size(), elements_.data(), wall_boundaries_.data(), faces_.data());
    }
    if (!symmetry_boundaries_.empty()) {
        SEM::Meshes::copy_boundaries_error(symmetry_boundaries_.size(), elements_.data(), symmetry_boundaries_.data(), faces_.data());
    }
    if (!inflow_boundaries_.empty()) {
        SEM::Meshes::copy_boundaries_error(inflow_boundaries_.size(), elements_.data(), inflow_boundaries_.data(), faces_.data());
    }
    if (!outflow_boundaries_.empty()) {
        SEM::Meshes::copy_boundaries_error(outflow_boundaries_.size(), elements_.data(), outflow_boundaries_.data(), faces_.data());
    }
    ...
    if (!wall_boundaries_.empty()) {
        SEM::Meshes::reduce_boundaries_refine_2D<boundaries_blockSize_/2>(wall_boundaries_.size(), elements_.data(), wall_boundaries_.data(), faces_.data(), device_wall_boundaries_refine_array_.data());
        device_wall_boundaries_refine_array_.copy_to(host_wall_boundaries_refine_array_, stream_);
    }

    if (!symmetry_boundaries_.empty()) {
        SEM::Meshes::reduce_boundaries_refine_2D<boundaries_blockSize_/2>(symmetry_boundaries_.size(), elements_.data(), symmetry_boundaries_.data(), faces_.data(), device_symmetry_boundaries_refine_array_.data());
        device_symmetry_boundaries_refine_array_.copy_to(host_symmetry_boundaries_refine_array_, stream_);
    }

    if (!inflow_boundaries_.empty()) {
        SEM::Meshes::reduce_boundaries_refine_2D<boundaries_blockSize_/2>(inflow_boundaries_.size(), elements_.data(), inflow_boundaries_.data(), faces_.data(), device_inflow_boundaries_refine_array_.data());
        device_inflow_boundaries_refine_array_.copy_to(host_inflow_boundaries_refine_array_, stream_);
    }

    if (!outflow_boundaries_.empty()) {
        SEM::Meshes::reduce_boundaries_refine_2D<boundaries_blockSize_/2>(outflow_boundaries_.size(), elements_.data(), outflow_boundaries_.data(), faces_.data(), device_outflow_boundaries_refine_array_.data());
        device_outflow_boundaries_refine_array_.copy_to(host_outflow_boundaries_refine_array_, stream_);
    }

    if (!interfaces_origin_.empty()) {
        SEM::Meshes::reduce_interfaces_refine_2D<boundaries_blockSize_/2>(interfaces_origin_.size(), max_split_level_, elements_.data(), interfaces_origin_.data(), device_interfaces_refine_array_.data());
        device_interfaces_refine_array_.copy_to(host_interfaces_refine_array_, stream_);
    }

    if (!mpi_interfaces_origin_.empty()) {
        SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(mpi_interfaces_origin_.size(), interfaces_refine_.data(), device_mpi_interfaces_outgoing_refine_array_.data());
        device_mpi_interfaces_outgoing_refine_array_.copy_to(host_mpi_interfaces_outgoing_refine_array_, stream_);
        
        SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(mpi_interfaces_destination_.size(), receiving_interfaces_refine_.data(), device_mpi_interfaces_incoming_refine_array_.data());
    ...
    // New arrays
    std::vector<Element2D_host_t> new_elements(elements_.size() + 3 * n_splitting_elements + n_splitting_wall_boundaries + n_splitting_symmetry_boundaries + n_splitting_inflow_boundaries + n_splitting_outflow_boundaries + n_splitting_interface_elements + n_splitting_mpi_interface_incoming_elements, stream_);
    std::vector<Vec2<hostFloat>> new_nodes(nodes_.size() + n_splitting_elements + n_splitting_faces, stream_);
    std::vector<Face2D_host_t> new_faces(faces_.size() + 4 * n_splitting_elements + n_splitting_faces, stream_);

    std::vector<size_t> new_wall_boundaries(wall_boundaries_.size() + n_splitting_wall_boundaries, stream_);
    std::vector<size_t> new_symmetry_boundaries(symmetry_boundaries_.size() + n_splitting_symmetry_boundaries, stream_);
    std::vector<size_t> new_inflow_boundaries(inflow_boundaries_.size() + n_splitting_inflow_boundaries, stream_);
    std::vector<size_t> new_outflow_boundaries(outflow_boundaries_.size() + n_splitting_outflow_boundaries, stream_);

    std::vector<size_t> new_interfaces_origin(interfaces_origin_.size() + n_splitting_interface_elements, stream_);
    std::vector<size_t> new_interfaces_origin_side(interfaces_origin_side_.size() + n_splitting_interface_elements, stream_);
    std::vector<size_t> new_interfaces_destination(interfaces_destination_.size() + n_splitting_interface_elements, stream_);

    std::vector<size_t> new_mpi_interfaces_origin(mpi_interfaces_origin_.size() + n_splitting_mpi_interface_outgoing_elements, stream_);
    std::vector<size_t> new_mpi_interfaces_origin_side(mpi_interfaces_origin_side_.size() + n_splitting_mpi_interface_outgoing_elements, stream_);
    std::vector<size_t> new_mpi_interfaces_destination(mpi_interfaces_destination_.size() + n_splitting_mpi_interface_incoming_elements, stream_);

    std::vector<size_t> elements_new_indices(elements_.size(), stream_);

    cudaMemcpyAsync(new_nodes.data(), nodes_.data(), nodes_.size() * sizeof(Vec2<hostFloat>), cudaMemcpyDeviceToDevice, stream_); // Apparently slower than using a kernel

    // Creating new entities and moving ond ones, adjusting as needed
    SEM::Meshes::hp_adapt(n_elements_, faces_.size(), nodes_.size(), n_splitting_elements, elements_.data(), new_elements.data(), faces_.data(), new_faces.data(), device_refine_array_.data(), device_faces_refine_array_.data(), max_split_level_, N_max, new_nodes.data(), polynomial_nodes.data(), barycentric_weights.data(), faces_blockSize_, elements_new_indices.data());
    
    if (!wall_boundaries_.empty()) {
        SEM::Meshes::split_boundaries(wall_boundaries_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements, elements_.data(), new_elements.data(), wall_boundaries_.data(), new_wall_boundaries.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_wall_boundaries_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, elements_new_indices.data());
    }
    if (!symmetry_boundaries_.empty()) {
        SEM::Meshes::split_boundaries(symmetry_boundaries_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements + wall_boundaries_.size() + n_splitting_wall_boundaries, elements_.data(), new_elements.data(), symmetry_boundaries_.data(), new_symmetry_boundaries.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_symmetry_boundaries_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, elements_new_indices.data());
    }
    if (!inflow_boundaries_.empty()) {
        SEM::Meshes::split_boundaries(inflow_boundaries_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements + wall_boundaries_.size() + n_splitting_wall_boundaries + symmetry_boundaries_.size() + n_splitting_symmetry_boundaries, elements_.data(), new_elements.data(), inflow_boundaries_.data(), new_inflow_boundaries.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_inflow_boundaries_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, elements_new_indices.data());
    }
    if (!outflow_boundaries_.empty()) {
        SEM::Meshes::split_boundaries(outflow_boundaries_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements + wall_boundaries_.size() + n_splitting_wall_boundaries + symmetry_boundaries_.size() + n_splitting_symmetry_boundaries + inflow_boundaries_.size() + n_splitting_inflow_boundaries, elements_.data(), new_elements.data(), outflow_boundaries_.data(), new_outflow_boundaries.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_outflow_boundaries_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, elements_new_indices.data());
    }

    if (!interfaces_origin_.empty()) {
        SEM::Meshes::split_interfaces(interfaces_origin_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements + wall_boundaries_.size() + n_splitting_wall_boundaries + symmetry_boundaries_.size() + n_splitting_symmetry_boundaries + inflow_boundaries_.size() + n_splitting_inflow_boundaries + outflow_boundaries_.size() + n_splitting_outflow_boundaries, elements_.data(), new_elements.data(), interfaces_origin_.data(), interfaces_origin_side_.data(), interfaces_destination_.data(), new_interfaces_origin.data(), new_interfaces_origin_side.data(), new_interfaces_destination.data(), faces_.data(), new_nodes.data(), device_refine_array_.data(), device_faces_refine_array_.data(), device_interfaces_refine_array_.data(), max_split_level_, N_max, polynomial_nodes.data(), elements_blockSize_, faces_blockSize_, elements_new_indices.data());
    }

    if (!mpi_interfaces_origin_.empty()) {       
        SEM::Meshes::split_mpi_outgoing_interfaces(mpi_interfaces_origin_.size(), elements_.data(), mpi_interfaces_origin_.data(), mpi_interfaces_origin_side_.data(), new_mpi_interfaces_origin.data(), new_mpi_interfaces_origin_side.data(), device_mpi_interfaces_outgoing_refine_array_.data(), max_split_level_, device_refine_array_.data(), elements_blockSize_);
        SEM::Meshes::split_mpi_incoming_interfaces(mpi_interfaces_destination_.size(), faces_.size(), nodes_.size(), n_splitting_elements, n_elements_ + 3 * n_splitting_elements + wall_boundaries_.size() + n_splitting_wall_boundaries + symmetry_boundaries_.size() + n_splitting_symmetry_boundaries + inflow_boundaries_.size() + n_splitting_inflow_boundaries + outflow_boundaries_.size() + n_splitting_outflow_boundaries + interfaces_origin_.size() + n_splitting_interface_elements, elements_.data(), new_elements.data(), mpi_interfaces_destination_.data(), new_mpi_interfaces_destination.data(), faces_.data(), new_nodes.data(), device_faces_refine_array_.data(), device_mpi_interfaces_incoming_refine_array_.data(), polynomial_nodes.data(), faces_blockSize_, receiving_interfaces_N_.data(), receiving_interfaces_refine_.data(), elements_new_indices.data());
    }
    ...
    std::vector<hostFloat> new_device_interfaces_p(mpi_interfaces_origin_.size() * (maximum_N_ + 1), stream_);
    std::vector<hostFloat> new_device_interfaces_u(mpi_interfaces_origin_.size() * (maximum_N_ + 1), stream_);
    std::vector<hostFloat> new_device_interfaces_v(mpi_interfaces_origin_.size() * (maximum_N_ + 1), stream_);
...
auto SEM::Meshes::Mesh2D_host_t::load_balance(const SEM::Entities::std::vector<hostFloat>& polynomial_nodes) -> void {
    int global_rank;
    MPI_Comm_rank(MPI_COMM_WORLD, &global_rank);
    int global_size;
    MPI_Comm_size(MPI_COMM_WORLD, &global_size);
    ...
    if (n_elements_send_left[global_rank] + n_elements_recv_left[global_rank] + n_elements_send_right[global_rank] + n_elements_recv_right[global_rank] > 0) {
        ...
        Element2D_host_t::Datatype element_data_type;

        // Sending and receiving
        std::vector<Element2D_host_t> elements_send_left(n_elements_send_left[global_rank]);
        std::vector<Element2D_host_t> elements_send_right(n_elements_send_right[global_rank]);
        ...
        if (n_elements_send_left[global_rank] > 0) {
            ...
            cudaMemcpyAsync(elements_send_left.data(), elements_.data(), n_elements_send_left[global_rank] * sizeof(Element2D_host_t), cudaMemcpyDeviceToHost, stream_);

            std::vector<hostFloat> solution_arrays(3 * n_elements_send_left[global_rank] * std::pow(maximum_N_ + 1, 2), stream_);
            std::vector<size_t> n_neighbours_arrays(4 * n_elements_send_left[global_rank], stream_);
            std::vector<hostFloat> nodes_arrays(8 * n_elements_send_left[global_rank], stream_);

            const int send_numBlocks = (n_elements_send_left[global_rank] + boundaries_blockSize_ - 1) / boundaries_blockSize_;
            SEM::Meshes::get_transfer_solution(n_elements_send_left[global_rank], elements_.data(), maximum_N_, nodes_.data(), solution_arrays.data(), n_neighbours_arrays.data(), nodes_arrays.data());
        ...
        if (n_elements_send_right[global_rank] > 0) {
            cudaMemcpyAsync(elements_send_right.data(), elements_.data() + n_elements_per_proc[global_rank] - n_elements_send_right[global_rank], n_elements_send_right[global_rank] * sizeof(Element2D_host_t), cudaMemcpyDeviceToHost, stream_);
            
            std::vector<hostFloat> solution_arrays(3 * n_elements_send_right[global_rank] * std::pow(maximum_N_ + 1, 2), stream_);
            std::vector<size_t> n_neighbours_arrays(4 * n_elements_send_right[global_rank], stream_);
            std::vector<hostFloat> nodes_arrays(8 * n_elements_send_right[global_rank], stream_);

            const int send_numBlocks = (n_elements_send_right[global_rank] + boundaries_blockSize_ - 1) / boundaries_blockSize_;
            SEM::Meshes::get_transfer_solution(n_elements_send_right[global_rank], elements_.data() + n_elements_per_proc[global_rank] - n_elements_send_right[global_rank], maximum_N_, nodes_.data(), solution_arrays.data(), n_neighbours_arrays.data(), nodes_arrays.data());
        ...
        if (n_elements_send_left[global_rank] > 0) {
            ...
            std::vector<size_t> neighbours_arrays(n_neighbours_send_total_left, stream_);
            std::vector<int> neighbours_proc_arrays(n_neighbours_send_total_left, stream_);
            std::vector<size_t> neighbours_side_arrays(n_neighbours_send_total_left, stream_);
            std::vector<size_t> neighbour_offsets_device(neighbour_offsets, stream_);
            std::vector<int> mpi_interfaces_new_process_incoming_device(mpi_interfaces_new_process_incoming, stream_);
            std::vector<size_t> mpi_interfaces_new_local_index_incoming_device(mpi_interfaces_new_local_index_incoming, stream_);
            std::vector<size_t> mpi_interfaces_new_side_incoming_device(mpi_interfaces_new_side_incoming, stream_);
            std::vector<size_t> n_elements_send_left_device(n_elements_send_left, stream_);
            std::vector<size_t> n_elements_recv_left_device(n_elements_recv_left, stream_);
            std::vector<size_t> n_elements_send_right_device(n_elements_send_right, stream_);
            std::vector<size_t> n_elements_recv_right_device(n_elements_recv_right, stream_);
            std::vector<size_t> global_element_offset_current_device(global_element_offset_current, stream_);
        ...
        if (n_elements_send_right[global_rank] > 0) {
            ...
            std::vector<size_t> neighbours_arrays(n_neighbours_send_total_right, stream_);
            std::vector<int> neighbours_proc_arrays(n_neighbours_send_total_right, stream_);
            std::vector<size_t> neighbours_side_arrays(n_neighbours_send_total_right, stream_);
            std::vector<size_t> neighbour_offsets_device(neighbour_offsets, stream_);
            std::vector<int> mpi_interfaces_new_process_incoming_device(mpi_interfaces_new_process_incoming, stream_);
            std::vector<size_t> mpi_interfaces_new_local_index_incoming_device(mpi_interfaces_new_local_index_incoming, stream_);
            std::vector<size_t> mpi_interfaces_new_side_incoming_device(mpi_interfaces_new_side_incoming, stream_);
        ...
        std::vector<Element2D_host_t> elements_recv_left(n_elements_recv_left[global_rank]);
        std::vector<Element2D_host_t> elements_recv_right(n_elements_recv_right[global_rank]);
        ...
        std::vector<size_t> received_node_indices(4 * (n_elements_recv_left[global_rank] + n_elements_recv_right[global_rank]), stream_);
        if (n_elements_recv_left[global_rank] + n_elements_recv_right[global_rank] > 0) {
            std::vector<bool> missing_received_nodes(received_node_indices.size(), stream_);
            std::vector<hostFloat> nodes_arrays_device(nodes_arrays_recv, stream_);
            const int received_nodes_numBlocks = (missing_received_nodes.size() + elements_blockSize_ - 1) / elements_blockSize_;

            SEM::Meshes::find_received_nodes(missing_received_nodes.size(), nodes_.size(), nodes_.data(), nodes_arrays_device.data(), missing_received_nodes.data(), received_node_indices.data());

            std::vector<size_t> device_missing_received_nodes_refine_array(received_nodes_numBlocks, stream_);
            SEM::Meshes::reduce_bools<elements_blockSize_/2>(missing_received_nodes.size(), missing_received_nodes.data(), device_missing_received_nodes_refine_array.data());
            ...
            std::vector<Vec2<hostFloat>> new_nodes(nodes_.size() + n_missing_received_nodes, stream_);
            cudaMemcpyAsync(new_nodes.data(), nodes_.data(), nodes_.size() * sizeof(Vec2<hostFloat>), cudaMemcpyDeviceToDevice, stream_); // Apparently slower than using a kernel

            SEM::Meshes::add_new_received_nodes(missing_received_nodes.size(), nodes_.size(), new_nodes.data(), nodes_arrays_device.data(), missing_received_nodes.data(), received_node_indices.data(), device_missing_received_nodes_refine_array.data());
        ...
        std::vector<Face2D_host_t> new_faces;

        // Faces to delete
        size_t n_faces_to_delete = 0;
        if (n_elements_send_left[global_rank] + n_elements_send_right[global_rank] > 0) {
            std::vector<bool> faces_to_delete(faces_.size(), stream_);
            SEM::Meshes::find_faces_to_delete(faces_.size(), n_elements_, n_elements_send_left[global_rank], n_elements_send_right[global_rank], faces_.data(), faces_to_delete.data());
        
            std::vector<size_t> device_faces_to_delete_refine_array(faces_numBlocks_, stream_);
            SEM::Meshes::reduce_bools<faces_blockSize_/2>(faces_to_delete.size(), faces_to_delete.data(), device_faces_to_delete_refine_array.data());
            ...
            std::vector<Face2D_host_t> new_new_faces(faces_.size() + n_faces_to_add - n_faces_to_delete, stream_);
            new_faces = std::move(new_new_faces);

            SEM::Meshes::move_required_faces(faces_.size(), faces_.data(), new_faces.data(), elements_.data(), faces_to_delete.data(), device_faces_to_delete_refine_array.data());
            ...
            std::vector<Face2D_host_t> new_new_faces(faces_.size() + n_faces_to_add, stream_);
            new_faces = std::move(new_new_faces);

            SEM::Meshes::move_faces(faces_.size(), faces_.data(), new_faces.data());
        ...
        std::vector<bool> boundary_elements_to_delete(elements_.size() - n_elements_, stream_);
        SEM::Meshes::find_boundary_elements_to_delete(elements_.size() - n_elements_, n_elements_, n_elements_send_left[global_rank], n_elements_send_right[global_rank], elements_.data(), new_faces.data(), boundary_elements_to_delete.data());

        if (n_elements_recv_left[global_rank] + n_elements_recv_right[global_rank] > 0) {
            SEM::Meshes::find_mpi_interface_elements_to_delete(mpi_interfaces_destination_.size(), n_elements_, global_rank, mpi_interfaces_destination_.data(), mpi_interfaces_new_process_incoming.data(), boundary_elements_to_delete.data());
        }
        std::vector<size_t> device_boundary_elements_to_delete_refine_array(ghosts_numBlocks_, stream_);
        SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(boundary_elements_to_delete.size(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data());
        ...
        std::vector<Element2D_host_t> new_elements(n_elements_new[global_rank] + elements_.size() - n_elements_ + n_boundary_elements_to_add - n_boundary_elements_to_delete, stream_); // What about boundary elements?
        ...
        SEM::Meshes::move_elements(n_elements_move, n_elements_send_left[global_rank], n_elements_recv_left[global_rank], elements_.data(), new_elements.data(), new_faces.data());

        SEM::Meshes::move_boundary_elements(boundary_elements_to_delete.size(), n_elements_, n_elements_new[global_rank], elements_.data(), new_elements.data(), new_faces.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data());
        ...
        if (n_elements_send_left[global_rank] + n_elements_send_right[global_rank] > 0) {
            if (!wall_boundaries_.empty()) {
                std::vector<bool> wall_boundaries_to_delete(wall_boundaries_.size(), stream_);
                SEM::Meshes::find_boundaries_to_delete(wall_boundaries_.size(), n_elements_, wall_boundaries_.data(), boundary_elements_to_delete.data(), wall_boundaries_to_delete.data());
                
                std::vector<size_t> device_wall_boundaries_to_delete_refine_array(wall_boundaries_numBlocks_, stream_);
                SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(wall_boundaries_to_delete.size(), wall_boundaries_to_delete.data(), device_wall_boundaries_to_delete_refine_array.data());
                ...
                std::vector<size_t> new_new_wall_boundaries(wall_boundaries_.size() + n_wall_boundaries_to_add - n_wall_boundaries_to_delete, stream_);
                new_wall_boundaries = std::move(new_new_wall_boundaries);

                SEM::Meshes::move_required_boundaries(wall_boundaries_.size(), n_elements_, n_elements_new[global_rank], wall_boundaries_.data(), new_wall_boundaries.data(), wall_boundaries_to_delete.data(), device_wall_boundaries_to_delete_refine_array.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
            ...
            if (!symmetry_boundaries_.empty()) {
                std::vector<bool> symmetry_boundaries_to_delete(symmetry_boundaries_.size(), stream_);
                SEM::Meshes::find_boundaries_to_delete(symmetry_boundaries_.size(), n_elements_, symmetry_boundaries_.data(), boundary_elements_to_delete.data(), symmetry_boundaries_to_delete.data());
                
                std::vector<size_t> device_symmetry_boundaries_to_delete_refine_array(symmetry_boundaries_numBlocks_, stream_);
                SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(symmetry_boundaries_to_delete.size(), symmetry_boundaries_to_delete.data(), device_symmetry_boundaries_to_delete_refine_array.data());
                ...
                std::vector<size_t> new_new_symmetry_boundaries(symmetry_boundaries_.size() + n_symmetry_boundaries_to_add - n_symmetry_boundaries_to_delete, stream_);
                new_symmetry_boundaries = std::move(new_new_symmetry_boundaries);

                SEM::Meshes::move_required_boundaries(symmetry_boundaries_.size(), n_elements_, n_elements_new[global_rank], symmetry_boundaries_.data(), new_symmetry_boundaries.data(), symmetry_boundaries_to_delete.data(), device_symmetry_boundaries_to_delete_refine_array.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
            ...
            if (!inflow_boundaries_.empty()) {
                std::vector<bool> inflow_boundaries_to_delete(inflow_boundaries_.size(), stream_);
                SEM::Meshes::find_boundaries_to_delete(inflow_boundaries_.size(), n_elements_, inflow_boundaries_.data(), boundary_elements_to_delete.data(), inflow_boundaries_to_delete.data());
                
                std::vector<size_t> device_inflow_boundaries_to_delete_refine_array(inflow_boundaries_numBlocks_, stream_);
                SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(inflow_boundaries_to_delete.size(), inflow_boundaries_to_delete.data(), device_inflow_boundaries_to_delete_refine_array.data());
            ...
            if (!outflow_boundaries_.empty()) {
                std::vector<bool> outflow_boundaries_to_delete(outflow_boundaries_.size(), stream_);
                SEM::Meshes::find_boundaries_to_delete(outflow_boundaries_.size(), n_elements_, outflow_boundaries_.data(), boundary_elements_to_delete.data(), outflow_boundaries_to_delete.data());
                
                std::vector<size_t> device_outflow_boundaries_to_delete_refine_array(outflow_boundaries_numBlocks_, stream_);
                SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(outflow_boundaries_to_delete.size(), outflow_boundaries_to_delete.data(), device_outflow_boundaries_to_delete_refine_array.data());
                ...
                std::vector<size_t> new_new_outflow_boundaries(outflow_boundaries_.size() + n_outflow_boundaries_to_add - n_outflow_boundaries_to_delete, stream_);
                new_outflow_boundaries = std::move(new_new_outflow_boundaries);

                SEM::Meshes::move_required_boundaries(outflow_boundaries_.size(), n_elements_, n_elements_new[global_rank], outflow_boundaries_.data(), new_outflow_boundaries.data(), outflow_boundaries_to_delete.data(), device_outflow_boundaries_to_delete_refine_array.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
            ...
            if (wall_boundaries_.size() + n_wall_boundaries_to_add > 0) {
                std::vector<size_t> new_new_wall_boundaries(wall_boundaries_.size() + n_wall_boundaries_to_add, stream_);
                new_wall_boundaries = std::move(new_new_wall_boundaries);

                SEM::Meshes::move_all_boundaries(wall_boundaries_.size(), n_elements_, n_elements_new[global_rank], wall_boundaries_.data(), new_wall_boundaries.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);

                new_new_wall_boundaries.clear(stream_);
            }

            if (symmetry_boundaries_.size() + n_symmetry_boundaries_to_add > 0) {
                std::vector<size_t> new_new_symmetry_boundaries(symmetry_boundaries_.size() + n_symmetry_boundaries_to_add, stream_);
                new_symmetry_boundaries = std::move(new_new_symmetry_boundaries);

                SEM::Meshes::move_all_boundaries(symmetry_boundaries_.size(), n_elements_, n_elements_new[global_rank], symmetry_boundaries_.data(), new_symmetry_boundaries.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);

                new_new_symmetry_boundaries.clear(stream_);
            }

            if (inflow_boundaries_.size() + n_inflow_boundaries_to_add > 0) {
                std::vector<size_t> new_new_inflow_boundaries(inflow_boundaries_.size() + n_inflow_boundaries_to_add, stream_);
                new_inflow_boundaries = std::move(new_new_inflow_boundaries);

                SEM::Meshes::move_all_boundaries(inflow_boundaries_.size(), n_elements_, n_elements_new[global_rank], inflow_boundaries_.data(), new_inflow_boundaries.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);

                new_new_inflow_boundaries.clear(stream_);
            }

            if (outflow_boundaries_.size() + n_outflow_boundaries_to_add > 0) {
                std::vector<size_t> new_new_outflow_boundaries(outflow_boundaries_.size() + n_outflow_boundaries_to_add, stream_);
                new_outflow_boundaries = std::move(new_new_outflow_boundaries);

                SEM::Meshes::move_all_boundaries(outflow_boundaries_.size(), n_elements_, n_elements_new[global_rank], outflow_boundaries_.data(), new_outflow_boundaries.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
        ...
        if (!mpi_interfaces_destination_.empty()) {
            std::vector<bool> mpi_destinations_to_delete(mpi_interfaces_destination_.size(), stream_);
            SEM::Meshes::find_boundaries_to_delete(mpi_interfaces_destination_.size(), n_elements_, mpi_interfaces_destination_.data(), boundary_elements_to_delete.data(), mpi_destinations_to_delete.data());
            
            std::vector<size_t> device_mpi_destinations_to_delete_refine_array(mpi_interfaces_incoming_numBlocks_, stream_);
            SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(mpi_destinations_to_delete.size(), mpi_destinations_to_delete.data(), device_mpi_destinations_to_delete_refine_array.data());
            ...
            std::vector<size_t> new_new_mpi_interfaces_destination(mpi_interfaces_destination_.size() + n_mpi_destinations_to_add - n_mpi_destinations_to_delete, stream_);
            new_mpi_interfaces_destination = std::move(new_new_mpi_interfaces_destination);

            SEM::Meshes::move_required_boundaries(mpi_interfaces_destination_.size(), n_elements_, n_elements_new[global_rank], mpi_interfaces_destination_.data(), new_mpi_interfaces_destination.data(), mpi_destinations_to_delete.data(), device_mpi_destinations_to_delete_refine_array.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
            ...
            std::vector<size_t> new_new_mpi_interfaces_destination(n_mpi_destinations_to_add, stream_);
            new_mpi_interfaces_destination = std::move(new_new_mpi_interfaces_destination);
        ...
        if (!mpi_interfaces_origin_.empty()) {
            std::vector<bool> mpi_origins_to_delete(mpi_interfaces_origin_.size(), stream_);
            SEM::Meshes::find_mpi_origins_to_delete(mpi_interfaces_origin_.size(), n_elements_, n_elements_send_left[global_rank], n_elements_send_right[global_rank], mpi_interfaces_origin_.data(), mpi_origins_to_delete.data());
            ...
            if (n_elements_recv_left[global_rank] + n_elements_recv_right[global_rank] > 0) {
                ...
                std::vector<int> mpi_origins_process(mpi_origins_process_host, stream_);
                SEM::Meshes::find_obstructed_mpi_origins_to_delete(mpi_interfaces_origin_.size(), n_elements_new[global_rank], n_elements_send_left[global_rank], n_elements_recv_left[global_rank], mpi_interfaces_destination_.size(), mpi_interfaces_origin_.data(), mpi_interfaces_origin_side_.data(), new_elements.data(), new_faces.data(), mpi_interfaces_destination_.data(), mpi_interfaces_new_process_incoming.data(), mpi_origins_process.data(), mpi_origins_to_delete.data(), boundary_elements_to_delete.size(), boundary_elements_to_delete.data());
            
                mpi_origins_process.clear(stream_);
            }

            std::vector<size_t> device_mpi_origins_to_delete_refine_array(mpi_interfaces_outgoing_numBlocks_, stream_);
            SEM::Meshes::reduce_bools<boundaries_blockSize_/2>(mpi_origins_to_delete.size(), mpi_origins_to_delete.data(), device_mpi_origins_to_delete_refine_array.data());
            ...
            std::vector<size_t> new_new_mpi_interfaces_origin(mpi_interfaces_origin_.size() + n_mpi_origins_to_add - n_mpi_origins_to_delete, stream_);
            new_mpi_interfaces_origin = std::move(new_new_mpi_interfaces_origin);
            std::vector<size_t> new_new_mpi_interfaces_origin_side(mpi_interfaces_origin_side_.size() + n_mpi_origins_to_add - n_mpi_origins_to_delete, stream_);
            new_mpi_interfaces_origin_side = std::move(new_new_mpi_interfaces_origin_side);

            // move the sides too please
            SEM::Meshes::move_required_mpi_origins(mpi_interfaces_origin_.size(), n_elements_, n_elements_new[global_rank], mpi_interfaces_origin_.data(), new_mpi_interfaces_origin.data(), mpi_interfaces_origin_side_.data(), new_mpi_interfaces_origin_side.data(), mpi_origins_to_delete.data(), device_mpi_origins_to_delete_refine_array.data(), boundary_elements_to_delete.data(), device_boundary_elements_to_delete_refine_array.data(), boundaries_blockSize_);
            ...
            std::vector<size_t> new_new_mpi_interfaces_origin(n_mpi_origins_to_add, stream_);
            std::vector<size_t> new_new_mpi_interfaces_origin_side(n_mpi_origins_to_add, stream_);
            new_mpi_interfaces_origin = std::move(new_new_mpi_interfaces_origin);
            new_mpi_interfaces_origin_side = std::move(new_new_mpi_interfaces_origin_side);
        ...
        if (n_elements_recv_left[global_rank] > 0) {
            ...
            cudaMemcpyAsync(new_elements.data(), elements_recv_left.data(), n_elements_recv_left[global_rank] * sizeof(Element2D_host_t), cudaMemcpyHostToDevice, stream_);
            std::vector<hostFloat> solution_arrays(solution_arrays_recv_left, stream_);
            std::vector<size_t> n_neighbours_arrays(n_neighbours_arrays_recv_left, stream_);

            const int recv_numBlocks = (n_elements_recv_left[global_rank] + boundaries_blockSize_ - 1) / boundaries_blockSize_;
            SEM::Meshes::fill_received_elements(n_elements_recv_left[global_rank], new_elements.data(), maximum_N_, nodes_.data(), solution_arrays.data(), n_neighbours_arrays.data(), received_node_indices.data(), polynomial_nodes.data());
        ...
        if (n_elements_recv_right[global_rank] > 0) {
            // Now we must store these elements
            cudaMemcpyAsync(new_elements.data() + n_elements_new[global_rank] - n_elements_recv_right[global_rank], elements_recv_right.data(), n_elements_recv_right[global_rank] * sizeof(Element2D_host_t), cudaMemcpyHostToDevice, stream_);
            std::vector<hostFloat> solution_arrays(solution_arrays_recv_right, stream_);
            std::vector<size_t> n_neighbours_arrays(n_neighbours_arrays_recv_right, stream_);

            const int recv_numBlocks = (n_elements_recv_right[global_rank] + boundaries_blockSize_ - 1) / boundaries_blockSize_;
            SEM::Meshes::fill_received_elements(n_elements_recv_right[global_rank], new_elements.data() + n_elements_new[global_rank] - n_elements_recv_right[global_rank], maximum_N_, nodes_.data(), solution_arrays.data(), n_neighbours_arrays.data(), received_node_indices.data() + 4 * n_elements_recv_left[global_rank], polynomial_nodes.data());
        ...
        std::vector<int> mpi_interfaces_new_process_incoming_device(mpi_interfaces_new_process_incoming, stream_);
        std::vector<size_t> mpi_interfaces_n_processes_device(mpi_interfaces_origin_.size(), stream_);

        const int mpi_outgoing_numBlocks = (mpi_interfaces_origin_.size() + boundaries_blockSize_ - 1) / boundaries_blockSize_;
        SEM::Meshes::get_interface_n_processes(mpi_interfaces_origin_.size(), mpi_interfaces_destination_.size(), elements_.data(), faces_.data(), mpi_interfaces_origin_.data(), mpi_interfaces_origin_side_.data(), mpi_interfaces_destination_.data(), mpi_interfaces_new_process_incoming_device.data(), mpi_interfaces_n_processes_device.data());
        ...
        std::vector<size_t> mpi_interfaces_process_offset_device(mpi_interfaces_process_offset_host, stream_);
        std::vector<int> mpi_interfaces_processes_device(interfaces_n_processes_total, stream_);

        SEM::Meshes::get_interface_processes(mpi_interfaces_origin_.size(), mpi_interfaces_destination_.size(), elements_.data(), faces_.data(), mpi_interfaces_origin_.data(), mpi_interfaces_origin_side_.data(), mpi_interfaces_destination_.data(), mpi_interfaces_new_process_incoming_device.data(), mpi_interfaces_process_offset_device.data(), mpi_interfaces_processes_device.data());
        ...
        std::vector<size_t> new_device_mpi_interfaces_outgoing_refine_array(mpi_interfaces_outgoing_numBlocks_, stream_);
        device_mpi_interfaces_outgoing_refine_array_ = std::move(new_device_mpi_interfaces_outgoing_refine_array);
        std::vector<hostFloat> new_device_interfaces_p(new_mpi_interfaces_origin.size() * (maximum_N_ + 1), stream_);
        std::vector<hostFloat> new_device_interfaces_u(new_mpi_interfaces_origin.size() * (maximum_N_ + 1), stream_);
        std::vector<hostFloat> new_device_interfaces_v(new_mpi_interfaces_origin.size() * (maximum_N_ + 1), stream_);
        device_interfaces_p_ = std::move(new_device_interfaces_p);
        device_interfaces_u_ = std::move(new_device_interfaces_u);
        device_interfaces_v_ = std::move(new_device_interfaces_v);
        host_interfaces_p_ = std::vector<hostFloat>(new_mpi_interfaces_origin.size() * (maximum_N_ + 1));
        host_interfaces_u_ = std::vector<hostFloat>(new_mpi_interfaces_origin.size() * (maximum_N_ + 1));
        host_interfaces_v_ = std::vector<hostFloat>(new_mpi_interfaces_origin.size() * (maximum_N_ + 1));
        std::vector<bool> new_device_interfaces_refine(new_mpi_interfaces_origin.size(), stream_);
        ...
        // Swap for new arrays
        std::vector<size_t> new_mpi_interfaces_origin_device(new_mpi_interfaces_origin, stream_);
        std::vector<size_t> new_mpi_interfaces_origin_side_device(new_mpi_interfaces_origin_side, stream_);
...
auto SEM::Meshes::Mesh2D_host_t::boundary_conditions(hostFloat t, const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& weights, const std::vector<hostFloat>& barycentric_weights) -> void {
    // Boundary conditions
    if (!wall_boundaries_.empty()) {
        SEM::Meshes::compute_wall_boundaries(wall_boundaries_.size(), elements_.data(), wall_boundaries_.data(), faces_.data(), polynomial_nodes.data(), weights.data(), barycentric_weights.data());
    }
    if (!symmetry_boundaries_.empty()) {
        SEM::Meshes::compute_symmetry_boundaries(symmetry_boundaries_.size(), elements_.data(), symmetry_boundaries_.data(), faces_.data(), polynomial_nodes.data(), weights.data(), barycentric_weights.data());
    }
    if (!inflow_boundaries_.empty()) {
        SEM::Meshes::compute_inflow_boundaries(inflow_boundaries_.size(), elements_.data(), inflow_boundaries_.data(), faces_.data(), t, nodes_.data(), polynomial_nodes.data());
    }
    if (!outflow_boundaries_.empty()) {
        SEM::Meshes::compute_outflow_boundaries(outflow_boundaries_.size(), elements_.data(), outflow_boundaries_.data(), faces_.data(), polynomial_nodes.data(), weights.data(), barycentric_weights.data());
    }

    // Interfaces
    if (!interfaces_origin_.empty()) {
        SEM::Meshes::local_interfaces(interfaces_origin_.size(), elements_.data(), interfaces_origin_.data(), interfaces_origin_side_.data(), interfaces_destination_.data());
    }
    ...
    if (!mpi_interfaces_origin_.empty()) {
        ...
        SEM::Meshes::get_MPI_interfaces(mpi_interfaces_origin_.size(), elements_.data(), mpi_interfaces_origin_.data(), mpi_interfaces_origin_side_.data(), maximum_N_, device_interfaces_p_.data(), device_interfaces_u_.data(), device_interfaces_v_.data());
        ...
        SEM::Meshes::put_MPI_interfaces(mpi_interfaces_destination_.size(), elements_.data(), mpi_interfaces_destination_.data(), maximum_N_, device_receiving_interfaces_p_.data(), device_receiving_interfaces_u_.data(), device_receiving_interfaces_v_.data());
...
__device__
auto SEM::Meshes::Mesh2D_host_t::almost_equal(hostFloat x, hostFloat y) -> bool {
...
auto SEM::Meshes::Mesh2D_host_t::interpolate_to_boundaries(const std::vector<hostFloat>& lagrange_interpolant_left, const std::vector<hostFloat>& lagrange_interpolant_right) -> void {
    SEM::Meshes::interpolate_to_boundaries(n_elements_, elements_.data(), lagrange_interpolant_left.data(), lagrange_interpolant_right.data());
}

auto SEM::Meshes::Mesh2D_host_t::project_to_faces(const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& barycentric_weights) -> void {
    SEM::Meshes::project_to_faces(faces_.size(), faces_.data(), elements_.data(), polynomial_nodes.data(), barycentric_weights.data());
}

auto SEM::Meshes::Mesh2D_host_t::project_to_elements(const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& weights, const std::vector<hostFloat>& barycentric_weights) -> void {
    SEM::Meshes::project_to_elements(n_elements_, faces_.data(), elements_.data(), polynomial_nodes.data(), weights.data(), barycentric_weights.data());
}

template auto SEM::Meshes::Mesh2D_host_t::estimate_error<SEM::Polynomials::ChebyshevPolynomial_host_t>(const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& weights) -> void;
template auto SEM::Meshes::Mesh2D_host_t::estimate_error<SEM::Polynomials::LegendrePolynomial_host_t>(const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& weights) -> void;

template<typename Polynomial>
auto SEM::Meshes::Mesh2D_host_t::estimate_error<Polynomial>(const std::vector<hostFloat>& polynomial_nodes, const std::vector<hostFloat>& weights) -> void {
    SEM::Meshes::estimate_error<Polynomial>(n_elements_, elements_.data(), tolerance_min_, tolerance_max_, polynomial_nodes.data(), weights.data());
}

auto SEM::Meshes::allocate_element_storage(size_t n_elements, Element2D_host_t* elements) -> void {
...
auto SEM::Meshes::allocate_boundary_storage(size_t n_domain_elements, size_t n_total_elements, Element2D_host_t* elements) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::compute_element_geometry(size_t n_elements, Element2D_host_t* elements, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::compute_boundary_geometry(size_t n_domain_elements, size_t n_total_elements, Element2D_host_t* elements, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::compute_element_status(size_t n_elements, Element2D_host_t* elements, const Vec2<hostFloat>* nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::allocate_face_storage(size_t n_faces, Face2D_host_t* faces) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::fill_element_faces(size_t n_elements, Element2D_host_t* elements, const std::array<size_t, 4>* element_to_face) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::fill_boundary_element_faces(size_t n_domain_elements, size_t n_total_elements, Element2D_host_t* elements, const std::array<size_t, 4>* element_to_face) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::compute_face_geometry(size_t n_faces, Face2D_host_t* faces, const Element2D_host_t* elements, const Vec2<hostFloat>* nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t face_index = index; face_index < n_faces; face_index += stride) {
        Face2D_host_t& face = faces[face_index];
        ...
        const Element2D_host_t& element_L = elements[element_L_index];
        const Element2D_host_t& element_R = elements[element_R_index];
...
auto SEM::Meshes::initial_conditions_2D(size_t n_elements, Element2D_host_t* elements, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t element_index = index; element_index < n_elements; element_index += stride) {
        ...
        const std::array<Vec2<hostFloat>, 4> points {nodes[element.nodes_[0]],
        ...
        for (int i = 0; i <= element.N_; ++i) {
            for (int j = 0; j <= element.N_; ++j) {
                ...
                const std::array<hostFloat, 3> state = SEM::g(global_coordinates, 0);
...
auto SEM::Meshes::get_solution(size_t n_elements, size_t n_interpolation_points, const Element2D_host_t* elements, const Vec2<hostFloat>* nodes, const hostFloat* interpolation_matrices, hostFloat* x, hostFloat* y, hostFloat* p, hostFloat* u, hostFloat* v) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
template auto SEM::Meshes::estimate_error<SEM::Polynomials::ChebyshevPolynomial_host_t>(size_t n_elements, Element2D_host_t* elements, hostFloat tolerance_min, hostFloat tolerance_max, const hostFloat* polynomial_nodes, const hostFloat* weights) -> void;
template auto SEM::Meshes::estimate_error<SEM::Polynomials::LegendrePolynomial_host_t>(size_t n_elements, Element2D_host_t* elements, hostFloat tolerance_min, hostFloat tolerance_max, const hostFloat* polynomial_nodes, const hostFloat* weights) -> void;

template<typename Polynomial>
auto SEM::Meshes::estimate_error<Polynomial>(size_t n_elements, Element2D_host_t* elements, hostFloat tolerance_min, hostFloat tolerance_max, const hostFloat* polynomial_nodes, const hostFloat* weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::interpolate_to_boundaries(size_t n_elements, Element2D_host_t* elements, const hostFloat* lagrange_interpolant_minus, const hostFloat* lagrange_interpolant_plus) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::project_to_faces(size_t n_faces, Face2D_host_t* faces, const Element2D_host_t* elements, const hostFloat* polynomial_nodes, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t face_index = index; face_index < n_faces; face_index += stride) {
        ...
        const Element2D_host_t& element_L = elements[face.elements_[0]];
        // Conforming
        if ((face.N_ == element_L.N_) 
            ...
            for (int i = 0; i <= face.N_; ++i) {
                const hostFloat coordinate = (polynomial_nodes[offset_1D + i] + 1) * face.scale_[0] + 2 * face.offset_[0] - 1;
                ...
                for (int j = 0; j <= element_L.N_; ++j) {
                    if (SEM::Meshes::Mesh2D_host_t::almost_equal(coordinate, polynomial_nodes[offset_1D_other + j])) {
                    ...
                    const hostFloat t = barycentric_weights[offset_1D_other + j]/(coordinate - polynomial_nodes[offset_1D_other + j]);
        ...
        const Element2D_host_t& element_R = elements[face.elements_[1]];
        ...
        if ((face.N_ == element_R.N_) 
            ...
            for (int i = 0; i <= face.N_; ++i) {
                const hostFloat coordinate = (polynomial_nodes[offset_1D + face.N_ - i] + 1) * face.scale_[1] + 2 * face.offset_[1] - 1;
...
auto SEM::Meshes::project_to_elements(size_t n_elements, const Face2D_host_t* faces, Element2D_host_t* elements, const hostFloat* polynomial_nodes, const hostFloat* weights, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t element_index = index; element_index < n_elements; element_index += stride) {
        ...
        for (size_t side_index = 0; side_index < element.faces_.size(); ++side_index) {
            ...
            if ((element.faces_[side_index].size() == 1)
                ...
                const Face2D_host_t& face = faces[element.faces_[side_index][0]];
            ...
            // Conforming, backwards
                ...
                for (size_t face_index = 0; face_index < element.faces_[side_index].size(); ++face_index) {
                    const Face2D_host_t& face = faces[element.faces_[side_index][face_index]];
                    const size_t offset_1D_other = face.N_ * (face.N_ + 1) /2;

                    // Non-conforming, forward
                    if (element_index == face.elements_[0]) {
                        for (int j = 0; j <= face.N_; ++j) {
                            const hostFloat coordinate = (polynomial_nodes[offset_1D_other + j] + 1) * face.scale_[0] + 2 * face.offset_[0] - 1;
                            bool found_row = false;
                            
                            for (int i = 0; i <= element.N_; ++i) {
                                if (SEM::Meshes::Mesh2D_host_t::almost_equal(coordinate, polynomial_nodes[offset_1D + i])) {
                            ...
                            if (!found_row) {
                                ...
                                for (int i = 0; i <= element.N_; ++i) {
                                    const hostFloat T = barycentric_weights[offset_1D + i]/((coordinate - polynomial_nodes[offset_1D + i]) * s);
                    ...
                    // Non-conforming, backwards
                        ...
                        for (int j = 0; j <= face.N_; ++j) {
                            const hostFloat coordinate = (polynomial_nodes[offset_1D_other + face.N_ - j] + 1) * face.scale_[1] + 2 * face.offset_[1] - 1;
                            bool found_row = false;
...
auto SEM::Meshes::compute_wall_boundaries(size_t n_wall_boundaries, Element2D_host_t* elements, const size_t* wall_boundaries, const Face2D_host_t* faces, const hostFloat* polynomial_nodes, const hostFloat* weights, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t boundary_index = index; boundary_index < n_wall_boundaries; boundary_index += stride) {
        ...
        if (element.faces_[0].size() == 0) { // Only one neighbour
            const Face2D_host_t& face = faces[element.faces_[0][0]];
            const int face_side = face.elements_[0] == element_index;
            const Element2D_host_t& neighbour = elements[face.elements_[face_side]];
            ...
            if (element.N_ == neighbour.N_) { // Conforming
                ...
                for (int j = 0; j <= neighbour.N_; ++j) {
                    const hostFloat coordinate = (polynomial_nodes[offset_1D_neighbour + neighbour.N_ - j] + 1) * face.scale_[face_side] + 2 * face.offset_[face_side] - 1;
                    bool found_row = false;
            ...
            for (size_t face_index = 0; face_index < element.faces_[0].size(); ++face_index) {
                const Face2D_host_t& face = faces[element.faces_[0][face_index]];
                const int face_side = face.elements_[0] == element_index;
                const Element2D_host_t& neighbour = elements[face.elements_[face_side]];
                const size_t neighbour_side = face.elements_side_[face_side];
                const Vec2<hostFloat> normal_inv {face.normal_.x(), face.tangent_.x()};
            ...
            const Face2D_host_t& face = faces[element.faces_[0][0]]; // CHECK this is kinda wrong, but we only use the normal and tangent so let's assume all the faces on a side have the same
            const Vec2<hostFloat> normal_inv {face.normal_.x(), face.tangent_.x()};
            const Vec2<hostFloat> tangent_inv {face.normal_.y(), face.tangent_.y()};
            for (int k = 0; k <= element.N_; ++k) {
                const Vec2<hostFloat> neighbour_velocity {element.u_extrapolated_[0][k], element.v_extrapolated_[0][k]};
...
auto SEM::Meshes::compute_symmetry_boundaries(size_t n_symmetry_boundaries, Element2D_host_t* elements, const size_t* symmetry_boundaries, const Face2D_host_t* faces, const hostFloat* polynomial_nodes, const hostFloat* weights, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::compute_inflow_boundaries(size_t n_inflow_boundaries, Element2D_host_t* elements, const size_t* inflow_boundaries, const Face2D_host_t* faces, hostFloat t, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t boundary_index = index; boundary_index < n_inflow_boundaries; boundary_index += stride) {
        Element2D_host_t& element = elements[inflow_boundaries[boundary_index]];
        const size_t offset_1D = element.N_ * (element.N_ + 1) /2;
        const std::array<Vec2<hostFloat>, 2> points{nodes[element.nodes_[0]], nodes[element.nodes_[1]]};

        for (int k = 0; k <= element.N_; ++k) {
            const hostFloat interp = (polynomial_nodes[offset_1D + k] + 1)/2;
            const Vec2<hostFloat> global_coordinates = points[1] * interp + points[0] * (1 - interp);

            const std::array<hostFloat, 3> state = SEM::g(global_coordinates, t);
...
auto SEM::Meshes::compute_outflow_boundaries(size_t n_outflow_boundaries, Element2D_host_t* elements, const size_t* outflow_boundaries, const Face2D_host_t* faces, const hostFloat* polynomial_nodes, const hostFloat* weights, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::local_interfaces(size_t n_local_interfaces, Element2D_host_t* elements, const size_t* local_interfaces_origin, const size_t* local_interfaces_origin_side, const size_t* local_interfaces_destination) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t interface_index = index; interface_index < n_local_interfaces; interface_index += stride) {
        const Element2D_host_t& source_element = elements[local_interfaces_origin[interface_index]];
        Element2D_host_t& destination_element = elements[local_interfaces_destination[interface_index]];
...
auto SEM::Meshes::get_MPI_interfaces(size_t n_MPI_interface_elements, const Element2D_host_t* elements, const size_t* MPI_interfaces_origin, const size_t* MPI_interfaces_origin_side, int maximum_N, hostFloat* p, hostFloat* u, hostFloat* v) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t interface_index = index; interface_index < n_MPI_interface_elements; interface_index += stride) {
        const Element2D_host_t& source_element = elements[MPI_interfaces_origin[interface_index]];
...
auto SEM::Meshes::get_MPI_interfaces_N(size_t n_MPI_interface_elements, int N_max, const Element2D_host_t* elements, const size_t* MPI_interfaces_origin, int* N) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::get_MPI_interfaces_adaptivity(size_t n_MPI_interface_elements, const Element2D_host_t* elements, const size_t* MPI_interfaces_origin, int* N, unsigned int* elements_splitting, int max_split_level, int N_max) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::put_MPI_interfaces(size_t n_MPI_interface_elements, Element2D_host_t* elements, const size_t* MPI_interfaces_destination, int maximum_N, const hostFloat* p, const hostFloat* u, const hostFloat* v) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t interface_index = index; interface_index < n_MPI_interface_elements; interface_index += stride) {
        Element2D_host_t& destination_element = elements[MPI_interfaces_destination[interface_index]];
        const size_t boundary_offset = interface_index * (maximum_N + 1);
...
auto SEM::Meshes::adjust_MPI_incoming_interfaces(size_t n_MPI_interface_elements, Element2D_host_t* elements, const size_t* MPI_interfaces_destination, const int* N, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::p_adapt(size_t n_elements, Element2D_host_t* elements, int N_max, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes, const hostFloat* barycentric_weights) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    
    for (size_t i = index; i < n_elements; i += stride) {
        if (elements[i].would_p_refine(N_max)) {
            Element2D_host_t new_element(elements[i].N_ + 2, elements[i].split_level_, elements[i].status_, elements[i].rotation_, elements[i].faces_, elements[i].nodes_);
...
auto SEM::Meshes::p_adapt_move(size_t n_elements, Element2D_host_t* elements, Element2D_host_t* new_elements, int N_max, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes, const hostFloat* barycentric_weights, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t i = index; i < n_elements; i += stride) {
        ...
        if (elements[i].would_p_refine(N_max)) {
            new_elements[i] = Element2D_host_t(elements[i].N_ + 2, elements[i].split_level_, elements[i].status_,  elements[i].rotation_, elements[i].faces_, elements[i].nodes_);
...
auto SEM::Meshes::p_adapt_split_faces(size_t n_elements, size_t n_faces, size_t n_nodes, size_t n_splitting_elements, Element2D_host_t* elements, Element2D_host_t* new_elements, const Face2D_host_t* faces, int N_max, const Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes, const hostFloat* barycentric_weights, const size_t* faces_block_offsets, int faces_blockSize, size_t* elements_new_indices) -> void {
    ...
    for (size_t element_index = index; element_index < n_elements; element_index += stride) {
        ...
        if (elements[element_index].would_p_refine(N_max)) {
           new_elements[element_index] = Element2D_host_t(element.N_ + 2, element.split_level_, element.status_, element.rotation_, element.faces_, element.nodes_);
...
auto SEM::Meshes::hp_adapt(size_t n_elements, size_t n_faces, size_t n_nodes, size_t n_splitting_elements, Element2D_host_t* elements, Element2D_host_t* new_elements, Face2D_host_t* faces, Face2D_host_t* new_faces, const size_t* block_offsets, const size_t* faces_block_offsets, int max_split_level, int N_max, Vec2<hostFloat>* nodes, const hostFloat* polynomial_nodes, const hostFloat* barycentric_weights, int faces_blockSize, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t i = index; i < n_elements; i += stride) {
        Element2D_host_t& element = elements[i];
        ...
        if (element.would_h_refine(max_split_level)) {
            ...
            for (size_t side_index = 0; side_index < element.nodes_.size(); ++side_index) {
                ...
                new_faces[new_face_index + side_index] = Face2D_host_t{element.N_, {new_node_indices[side_index], new_node_index}, side_element_indices, side_element_sides};
            
                std::array<std::vector<size_t>, 4> new_element_faces {1, 1, 1, 1};
                ...
                if (element.additional_nodes_[side_index]) {
                    ...
                    for (size_t side_face_index = 0; side_face_index < element.faces_[side_index].size(); ++side_face_index) {
                        const size_t face_index = element.faces_[side_index][side_face_index];
                        const Face2D_host_t& face = faces[face_index];
                        if (face.refine_) {
                ...
                new_elements[element_index + child_order[side_index]] = Element2D_host_t(element.N_, element.split_level_ + 1, child_statuses[side_index], element.rotation_, new_element_faces, new_element_node_indices);
...
auto SEM::Meshes::split_faces(size_t n_faces, size_t n_nodes, size_t n_splitting_elements, Face2D_host_t* faces, Face2D_host_t* new_faces, const Element2D_host_t* elements, Vec2<hostFloat>* nodes, const size_t* faces_block_offsets, int max_split_level, int N_max, const size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
    ...
    for (size_t face_index = index; face_index < n_faces; face_index += stride) {
        ...
        if (face.refine_) {
            ...
            new_faces[face_index] = Face2D_host_t(face_N, {face.nodes_[0], new_node_index}, new_element_indices[0], face.elements_side_);
            new_faces[new_face_index] = Face2D_host_t(face_N, {new_node_index, face.nodes_[1]}, new_element_indices[1], face.elements_side_);
...
auto SEM::Meshes::find_nodes(size_t n_elements, Element2D_host_t* elements, const Face2D_host_t* faces, const Vec2<hostFloat>* nodes, int max_split_level) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::no_new_nodes(size_t n_elements, Element2D_host_t* elements) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::copy_boundaries_error(size_t n_boundaries, Element2D_host_t* elements, const size_t* boundaries, const Face2D_host_t* faces) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t boundary_index = index; boundary_index < n_boundaries; boundary_index += stride) {
        ...
        Element2D_host_t& destination_element = elements[destination_element_index];

        if (destination_element.faces_[0].size() == 1) { // Should always be the case 
            const Face2D_host_t& face = faces[destination_element.faces_[0][0]];
            const size_t face_side = face.elements_[0] == destination_element_index;
            const Element2D_host_t& source_element = elements[face.elements_[face_side]];
...
auto SEM::Meshes::copy_interfaces_error(size_t n_local_interfaces, Element2D_host_t* elements, const size_t* local_interfaces_origin, const size_t* local_interfaces_origin_side, const size_t* local_interfaces_destination) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::copy_mpi_interfaces_error(size_t n_MPI_interface_elements, Element2D_host_t* elements, const Face2D_host_t* faces, const Vec2<hostFloat>* nodes, const size_t* MPI_interfaces_destination, const int* N, const unsigned int* elements_splitting) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t interface_index = index; interface_index < n_MPI_interface_elements; interface_index += stride) {
        Element2D_host_t& element = elements[MPI_interfaces_destination[interface_index]];
...
auto SEM::Meshes::split_boundaries(size_t n_boundaries, size_t n_faces, size_t n_nodes, size_t n_splitting_elements, size_t offset, Element2D_host_t* elements, Element2D_host_t* new_elements, const size_t* boundaries, size_t* new_boundaries, const Face2D_host_t* faces, const Vec2<hostFloat>* nodes, const size_t* faces_block_offsets, const size_t* boundary_block_offsets, const hostFloat* polynomial_nodes, int faces_blockSize, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
    ...
    for (size_t boundary_index = index; boundary_index < n_boundaries; boundary_index += stride) {
        ...
        Element2D_host_t& destination_element = elements[element_index];
        ...
        if ((elements[element_index].faces_[0].size() == 1) && (faces[elements[element_index].faces_[0][0]].refine_)) {
            ...
            for (size_t face_index = 0; face_index < destination_element.faces_[0].size(); ++face_index) {
                const Face2D_host_t& face = faces[destination_element.faces_[0][face_index]];
                const int neighbour_element_index = face.elements_[0] == element_index;
                const Element2D_host_t& source_element = elements[face.elements_[neighbour_element_index]];
...
auto SEM::Meshes::split_interfaces(size_t n_local_interfaces, size_t n_faces, size_t n_nodes, size_t n_splitting_elements, size_t offset, Element2D_host_t* elements, Element2D_host_t* new_elements, const size_t* local_interfaces_origin, const size_t* local_interfaces_origin_side, const size_t* local_interfaces_destination, size_t* new_local_interfaces_origin, size_t* new_local_interfaces_origin_side, size_t* new_local_interfaces_destination, const Face2D_host_t* faces, const Vec2<hostFloat>* nodes, const size_t* block_offsets, const size_t* faces_block_offsets, const size_t* interface_block_offsets, int max_split_level, int N_max, const hostFloat* polynomial_nodes, int elements_blockSize, int faces_blockSize, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
    ...
    for (size_t interface_index = index; interface_index < n_local_interfaces; interface_index += stride) {
        ...
        const Element2D_host_t& source_element = elements[source_element_index];
        Element2D_host_t& destination_element = elements[destination_element_index];
...
auto SEM::Meshes::split_mpi_outgoing_interfaces(size_t n_MPI_interface_elements, const Element2D_host_t* elements, const size_t* mpi_interfaces_origin, const size_t* mpi_interfaces_origin_side, size_t* new_mpi_interfaces_origin, size_t* new_mpi_interfaces_origin_side, const size_t* mpi_interface_block_offsets, int max_split_level, const size_t* block_offsets, int elements_blockSize) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
    ...
    for (size_t mpi_interface_index = index; mpi_interface_index < n_MPI_interface_elements; mpi_interface_index += stride) {
        ...
        const Element2D_host_t& origin_element = elements[origin_element_index];
...
auto SEM::Meshes::split_mpi_incoming_interfaces(size_t n_MPI_interface_elements, size_t n_faces, size_t n_nodes, size_t n_splitting_elements, size_t offset, Element2D_host_t* elements, Element2D_host_t* new_elements, const size_t* mpi_interfaces_destination, size_t* new_mpi_interfaces_destination, const Face2D_host_t* faces, const Vec2<hostFloat>* nodes, const size_t* faces_block_offsets, const size_t* mpi_interface_block_offsets, const hostFloat* polynomial_nodes, int faces_blockSize, const int* N, const unsigned int* elements_splitting, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
...
auto SEM::Meshes::adjust_boundaries(size_t n_boundaries, Element2D_host_t* elements, const size_t* boundaries, const Face2D_host_t* faces) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t boundary_index = index; boundary_index < n_boundaries; boundary_index += stride) {
        Element2D_host_t& destination_element = elements[boundaries[boundary_index]];
        int N_element = destination_element.N_;

        for (size_t face_index = 0; face_index < destination_element.faces_[0].size(); ++face_index) {
            const Face2D_host_t& face = faces[destination_element.faces_[0][face_index]];
            const int element_index = face.elements_[0] == boundaries[boundary_index];
            const Element2D_host_t& source_element = elements[face.elements_[element_index]];
...
auto SEM::Meshes::adjust_interfaces(size_t n_local_interfaces, Element2D_host_t* elements, const size_t* local_interfaces_origin, const size_t* local_interfaces_destination) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::adjust_faces(size_t n_faces, Face2D_host_t* faces, const Element2D_host_t* elements) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::adjust_faces_neighbours(size_t n_faces, Face2D_host_t* faces, const Element2D_host_t* elements, const Vec2<hostFloat>* nodes, int max_split_level, int N_max, const size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::move_boundaries(size_t n_boundaries, Element2D_host_t* elements, Element2D_host_t* new_elements, const size_t* boundaries, const Face2D_host_t* faces, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t boundary_index = index; boundary_index < n_boundaries; boundary_index += stride) {
        const size_t boundary_element_index = boundaries[boundary_index];
        Element2D_host_t& destination_element = elements[boundary_element_index];
        int N_element = destination_element.N_;
...
auto SEM::Meshes::move_interfaces(size_t n_local_interfaces, Element2D_host_t* elements, Element2D_host_t* new_elements, const size_t* local_interfaces_origin, const size_t* local_interfaces_destination, size_t* elements_new_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::get_transfer_solution(size_t n_elements, const Element2D_host_t* elements, int maximum_N, const Vec2<hostFloat>* nodes, hostFloat* solution, size_t* n_neighbours, hostFloat* element_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::fill_received_elements(size_t n_elements, Element2D_host_t* elements, int maximum_N, const Vec2<hostFloat>* nodes, const hostFloat* solution, const size_t* n_neighbours, const size_t* received_node_indices, const hostFloat* polynomial_nodes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::get_neighbours(size_t n_elements_send, 
                                 ...
                                 const Element2D_host_t* elements, 
                                 const Face2D_host_t* faces, 
    ...
    for (size_t element_index = index; element_index < n_elements_send; element_index += stride) {
        const Element2D_host_t& element = elements[element_index + start_index];
                        ...
                        if (missing) {
                            for (size_t i = 0; i < n_wall_boundaries; ++i) {
                                if (wall_boundaries[i] == other_element_index) {
                                    ...
                                    neighbours_proc[element_offset] = SEM::Meshes::Mesh2D_host_t::boundary_type::wall;
                            ...
                            if (missing) {
                                for (size_t i = 0; i < n_symmetry_boundaries; ++i) {
                                    if (symmetry_boundaries[i] == other_element_index) {
                                        neighbours[element_offset] = static_cast<size_t>(-1);
                                        neighbours_proc[element_offset] = SEM::Meshes::Mesh2D_host_t::boundary_type::symmetry;
                                        neighbours_side[element_offset] = static_cast<size_t>(-1);
                                ...
                                if (missing) {
                                    for (size_t i = 0; i < n_inflow_boundaries; ++i) {
                                        if (inflow_boundaries[i] == other_element_index) {
                                            neighbours[element_offset] = static_cast<size_t>(-1);
                                            neighbours_proc[element_offset] = SEM::Meshes::Mesh2D_host_t::boundary_type::inflow;
                                            neighbours_side[element_offset] = static_cast<size_t>(-1);
                                    ...
                                    if (missing) {
                                        for (size_t i = 0; i < n_outflow_boundaries; ++i) {
                                            if (outflow_boundaries[i] == other_element_index) {
                                                neighbours[element_offset] = static_cast<size_t>(-1);
                                                neighbours_proc[element_offset] = SEM::Meshes::Mesh2D_host_t::boundary_type::outflow;
                                                neighbours_side[element_offset] = static_cast<size_t>(-1);
...
auto SEM::Meshes::move_elements(size_t n_elements_move, size_t n_elements_send_left, size_t n_elements_recv_left, Element2D_host_t* elements, Element2D_host_t* new_elements, Face2D_host_t* faces) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t element_index = index; element_index < n_elements_move; element_index += stride) {
        ...
        for (size_t i = 0; i < new_elements[destination_element_index].faces_.size(); ++i) {
            for (size_t j = 0; j < new_elements[destination_element_index].faces_[i].size(); ++j) {
                Face2D_host_t face = faces[new_elements[destination_element_index].faces_[i][j]];
...
auto SEM::Meshes::get_interface_n_processes(size_t n_mpi_interfaces, size_t n_mpi_interfaces_incoming, const Element2D_host_t* elements, const Face2D_host_t* faces, const size_t* mpi_interfaces_origin, const size_t* mpi_interfaces_origin_side, const size_t* mpi_interfaces_destination, const int* mpi_interfaces_new_process_incoming, size_t* n_processes) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    ...
    for (size_t interface_index = index; interface_index < n_mpi_interfaces; interface_index += stride) {
        ...
        for (size_t i = 0; i < n_faces; ++i) {
            ...
            for (size_t j = 0; j < i; ++j) {
                ...
                const Face2D_host_t& previous_face = faces[previous_face_index];
...
auto SEM::Meshes::find_received_nodes(size_t n_received_nodes, size_t n_nodes, const Vec2<hostFloat>* nodes, const hostFloat* received_nodes, bool* missing_received_nodes, size_t* received_nodes_indices) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::add_new_received_nodes(size_t n_received_nodes, size_t n_nodes, Vec2<hostFloat>* nodes, const hostFloat* received_nodes, const bool* missing_received_nodes, size_t* received_nodes_indices, const size_t* received_nodes_block_offsets) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
...
auto SEM::Meshes::find_faces_to_delete(size_t n_faces, size_t n_domain_elements, size_t n_elements_send_left, size_t n_elements_send_right, const Face2D_host_t* faces, bool* faces_to_delete) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;

    for (size_t i = index; i < n_faces; i += stride) {
        const Face2D_host_t& face = faces[i];
...
auto SEM::Meshes::move_faces(size_t n_faces, Face2D_host_t* faces, Face2D_host_t* new_faces) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::move_required_faces(size_t n_faces, Face2D_host_t* faces, Face2D_host_t* new_faces, Element2D_host_t* elements, const bool* faces_to_delete, const size_t* faces_to_delete_block_offsets) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
    ...
    for (size_t i = index; i < n_faces; i += stride) {
        if (!faces_to_delete[i]) {
            ...
            Element2D_host_t& element_L = elements[new_faces[new_face_index].elements_[0]];
            Element2D_host_t& element_R = elements[new_faces[new_face_index].elements_[1]];
            ...
            Element2D_host_t& element_L = elements[faces[i].elements_[0]];
            Element2D_host_t& element_R = elements[faces[i].elements_[1]];
...
auto SEM::Meshes::find_boundary_elements_to_delete(size_t n_boundary_elements, size_t n_domain_elements, size_t n_elements_send_left, size_t n_elements_send_right, const Element2D_host_t* elements, const Face2D_host_t* faces, bool* boundary_elements_to_delete) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
...
auto SEM::Meshes::move_boundary_elements(size_t n_boundary_elements, size_t n_domain_elements, size_t new_n_domain_elements, Element2D_host_t* elements, Element2D_host_t* new_elements, Face2D_host_t* faces, const bool* boundary_elements_to_delete, const size_t* boundary_elements_to_delete_block_offsets) -> void {
    const int index = blockIdx.x * blockDim.x + threadIdx.x;
    const int stride = blockDim.x * gridDim.x;
    const int thread_id = threadIdx.x;
    const int block_id = blockIdx.x;
