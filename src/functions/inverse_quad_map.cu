...
auto SEM::inverse_quad_map(Vec2<deviceFloat> global_coordinates, const std::array<Vec2<deviceFloat>, 4>& points) -> Vec2<deviceFloat> {
    ...
    if(std::abs(k2) < 0.001) {
        const deviceFloat u_denominator_x = e.x() * k1 - g.x() * k0;
        const deviceFloat u_denominator_y = e.y() * k1 - g.y() * k0;

        const deviceFloat u = (std::abs(u_denominator_x) > std::abs(u_denominator_y)) ? (h.x() * k1 + f.x() * k0) / u_denominator_x : (h.y() * k1 + f.y() * k0) / u_denominator_y;
    ...
    {
        deviceFloat w = k1 * k1 - 4 * k0 * k2;
        if(w < 0.0) return {-1, -1};
        w = std::sqrt(w);

        const deviceFloat ik2 = 1 / (2 * k2);
        deviceFloat v = (-k1 - w) * ik2;

        deviceFloat u_denominator_x = e.x() + g.x() * v;
        deviceFloat u_denominator_y = e.y() + g.y() * v; 

        deviceFloat u = (std::abs(u_denominator_x) > std::abs(u_denominator_y)) ? (h.x() - f.x() * v) / u_denominator_x : (h.y() - f.y() * v) / u_denominator_y;
        
        if(u < static_cast<deviceFloat>(0.0) || u > static_cast<deviceFloat>(1.0) || v < static_cast<deviceFloat>(0.0) || v > static_cast<deviceFloat>(1.0)) {
            v = (-k1 + w) * ik2;
