...
TEST_CASE("Initial conditions solution value", "Checks the node values are correct after initial conditions.") {   
    ...
    SEM::Device::Meshes::Mesh_t mesh(N_elements, N_test, delta_x_min, x_span[0], x_span[1], adaptivity_interval, stream);
    ...
    SEM::Device::Entities::get_solution<<<mesh.elements_numBlocks_, mesh.elements_blockSize_, 0, stream>>>(mesh.N_elements_, n_interpolation_points, mesh.elements_, NDG.interpolation_matrices_.data(), x, phi, phi_prime, intermediate, x_L, x_R, N, sigma, refine, coarsen, error, delta_x);
    ...
    for (int i = 0; i < N_elements * n_interpolation_points; ++i) {
        const double expected = SEM::Device::Entities::g(host_x[i]);
...
TEST_CASE("Initial conditions boundary values", "Checks the extrapolated boundary values are correct after initial conditions.") {   
    ...
    SEM::Device::Meshes::Mesh_t mesh(N_elements, N_test, delta_x_min, x_span[0], x_span[1], adaptivity_interval, stream);
    mesh.set_initial_conditions(NDG.nodes_.data());
    SEM::Device::Entities::interpolate_to_boundaries<<<mesh.elements_numBlocks_, mesh.elements_blockSize_, 0, stream>>>(mesh.N_elements_, mesh.elements_, NDG.lagrange_interpolant_left_.data(), NDG.lagrange_interpolant_right_.data());
    mesh.boundary_conditions();
    SEM::Device::Meshes::calculate_fluxes<<<mesh.faces_numBlocks_, mesh.faces_blockSize_, 0, stream>>>(mesh.N_faces_, mesh.faces_, mesh.elements_);
    SEM::Device::Meshes::compute_dg_derivative<<<mesh.elements_numBlocks_, mesh.elements_blockSize_, 0, stream>>>(mesh.N_elements_, mesh.elements_, mesh.faces_, NDG.weights_.data(), NDG.derivative_matrices_hat_.data(), NDG.lagrange_interpolant_left_.data(), NDG.lagrange_interpolant_right_.data());
    SEM::Device::Entities::interpolate_q_to_boundaries<<<mesh.elements_numBlocks_, mesh.elements_blockSize_, 0, stream>>>(mesh.N_elements_, mesh.elements_, NDG.lagrange_interpolant_left_.data(), NDG.lagrange_interpolant_right_.data());

    std::vector<SEM::Device::Entities::Face_t> host_faces(mesh.N_faces_);
    std::vector<SEM::Device::Entities::Element_t> host_elements(mesh.N_elements_ + mesh.N_local_boundaries_ + mesh.N_MPI_boundaries_);
    cudaMemcpy(host_faces.data(), mesh.faces_, mesh.N_faces_ * sizeof(SEM::Device::Entities::Face_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(host_elements.data(), mesh.elements_, (mesh.N_elements_ + mesh.N_local_boundaries_ + mesh.N_MPI_boundaries_) * sizeof(SEM::Device::Entities::Element_t), cudaMemcpyDeviceToHost);
    ...
    for (int i = 0; i < N_elements; ++i) {
        const double phi_L_expected = SEM::Device::Entities::g(host_elements[i].x_[0]);
        const double phi_R_expected = SEM::Device::Entities::g(host_elements[i].x_[1]);
    ...
    for (int i = 0; i < N_elements; ++i) {
        const double phi_prime_L_expected = SEM::Device::Entities::g_prime(host_elements[i].x_[0]);
        const double phi_prime_R_expected = SEM::Device::Entities::g_prime(host_elements[i].x_[1]);
