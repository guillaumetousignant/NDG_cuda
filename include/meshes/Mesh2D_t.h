...
namespace SEM { namespace Host { namespace Meshes {
    ...
    // From https://developer.download.nvidia.com/assets/cuda/files/reduction.pdf
    template <unsigned int blockSize>
    auto reduce_refine_2D(size_t n_elements, int max_split_level, const SEM::Host::Entities::Element2D_t* elements, size_t* g_odata) -> void {
    ...
    // From https://developer.download.nvidia.com/assets/cuda/files/reduction.pdf
    template <unsigned int blockSize>
    auto reduce_interfaces_refine_2D(size_t n_local_interfaces, int max_split_level, const SEM::Host::Entities::Element2D_t* elements, const size_t* local_interfaces_origin, size_t* g_odata) -> void {
        size_t sdata[(blockSize >= 64) ? blockSize : blockSize + blockSize/2]; // Because within a warp there is no branching and this is read up until blockSize + blockSize/2
        unsigned int tid = 0;
        size_t i = 0*(blockSize*2) + tid;
        unsigned int gridSize = blockSize*2*0;
