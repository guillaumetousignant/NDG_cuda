...
\begin{figure}[H]
    ...
    \caption{\Acrlong{acr:AMR} performance: Simulation time and error for increasing number increasing number of pre-condition steps. \(N_{initial} = 4\), \(K_{initial} = 16\), \(S = 5\), refinement interval = 5}\label{fig:adaptivity_efficiency_A5}
\end{figure}
...
able to reach a maximum error only 14\% greater than the non-adaptive case, while being 34\% faster.
The other cases also attain the target error estimate of \num{1e-6} while being faster.
...
price of a worse error that still stays below the target error estimate we set.
