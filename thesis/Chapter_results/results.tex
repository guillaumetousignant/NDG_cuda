...
The optimal number of pre-processing steps seems to be three for this case. After this number of
steps the solution doesn't change much as the error estimation target is likely reached.

\begin{figure}[H]
	\centering
	\subfloat[Solution time]
	{\includesvg[width=0.45\textwidth]{Chapter_results/media/adaptivity_time_N4_K16_C3}\label{fig:adaptivity_efficiency_C3_time}}
	\hfill
	\subfloat[Maximum error]
	{\includesvg[width=0.45\textwidth]{Chapter_results/media/adaptivity_error_N4_K16_C3}\label{fig:adaptivity_efficiency_C3_error}}
	\caption{\Acrlong{acr:AMR} performance: Simulation time and analytical error with 3 pre-processing steps and increasing refinement interval. N = 4, K = 16, S = 5, pre-processing steps = 3}\label{fig:adaptivity_efficiency_C3}
\end{figure}
