...
two blocks for two \acrshortpl{acr:GPU} to work in parallel, as shown in
...
One type of error that cannot be alleviated by \acrshort{acr:AMR} is error caused by the application
...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[node distance=1.5cm]
        \node (start) [start, align=center] {Pre-condition\\start};
        \node (init) [process, right of=start, xshift=2cm, align=center] {Apply initial\\conditions};
        \node (n_steps) [decision, right of=init, xshift=2.5cm, align=center] {Number of\\pre-condition\\steps reached?};
        \node (timestep) [process, below of=n_steps, yshift=-1.5cm, align=center] {Perform\\timesteps};
        \node (refine) [process, below of=init, yshift=-1.5cm, align=center] {\Acrshort{acr:AMR}};
        \node (compute) [start, right of=n_steps, xshift=2.5cm, align=center] {Start\\computation};

        \draw [arrow] (start) -- (init);
        \draw [arrow] (init) -- (n_steps);
        \draw [arrow] (n_steps) -- node[anchor=north] {yes} (compute);
        \draw [arrow] (n_steps) -- node[anchor=west] {no} (timestep);
        \draw [arrow] (timestep) -- (refine);
        \draw [arrow] (refine) -- (init);
    \end{tikzpicture}
