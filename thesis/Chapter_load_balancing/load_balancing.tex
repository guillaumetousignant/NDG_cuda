...
\begin{figure}[H]
	...
	{\includesvg[width=0.45\textwidth]{Chapter_load_balancing/media/mesh_1_before_adaptivity0} \label{fig:hilbert_before}}
	\hfill
	\subfloat[After refining]
	{\includesvg[width=0.45\textwidth]{Chapter_load_balancing/media/mesh_1_after_adaptivity0} \label{fig:hilbert_after}}
...
% Insert figure with the weird cases and the splits
...
Until now, we assumed the elements where axis-aligned, and numbered such that their first edge is at
...
situation is expected to happen.

% Insert figure with rotated elements
...
% Insert figure with local referential numbering
...
With this out of the way, it is possible to store the element once when the mesh is read, store it
