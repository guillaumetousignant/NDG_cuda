...
The high-order orthogonal polynomials we use have associated weights \(w\) that ensures
orthogonality of the polynomials. \(D_{ij}\) is a constant.
...
\begin{figure}[H]
	...
	\caption{Polynomial interpolation: The Lagrange integrating polynomials of degree six.}\label{fig:interpolants}
\end{figure}
...
pointing outwards. Applied to Equation~\ref{equ:2d_wave_inner_product}'s second term, we get:
...
integrating polynomial \(l_i\) is equal to 1 and not 0 on each collocation point \(x_k\) to get:
