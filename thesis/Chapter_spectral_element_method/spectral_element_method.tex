...
Figure~\ref{fig:domain_nodes}, and the boundary term calculated collocation points on the element
...
Since it is not evident what the upwind state of the system in Equation~\ref{equ:physical_fluxes},
we will decouple the wave components. We decompose \(A\) into a diagonal matrix \(\Lambda \), its
right eigenvectors \(S\), and their inverse \(S^{-1}\).
