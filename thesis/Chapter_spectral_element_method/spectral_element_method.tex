...
By combining $\overrightarrow{f}$ and $\overrightarrow{g}$ into a flux vector $\mathbf{F}$, we get 
the conservative form of the wave equation.

\begin{equation}
	\mathbf{F} = \overrightarrow{f}\widehat{x} + \overrightarrow{g}\widehat{y}
\end{equation}
...
\begin{equation} \label{equ:2d_wave_conservation}
	\overrightarrow{q_t} + \bigtriangledown \cdot \mathbf{F} = 0
\end{equation}
...
\begin{equation} \label{equ:2d_wave_integral}
	\frac{d}{dt}\int_{V}\overrightarrow{q}dV = -\int_{S}\mathbf{F}\cdot \widehat{n}dS. 
\end{equation}
...
\begin{figure}[H]
	\centering
	\includesvg[width=0.8\textwidth]{Chapter_spectral_element_method/media/domain}
