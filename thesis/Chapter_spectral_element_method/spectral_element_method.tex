...
methods use the Gauss-Lobatto quadrature points to make sure the solution is continuous at element
...
the intended work on \acrshortpl{acr:GPU}. It is accurate and geometrically flexible. It is also
easy to parallelise, as the discontinuities between elements make them independent. Each element can
be computed independently in parallel, then the fluxes are also computed in parallel. The flux
formulation also simplifies working with non-conforming interfaces, such as those resulting of
\acrlong{acr:AMR}, as seen in Section~\ref{section:adaptive_mesh_refinement:mortar_element_method}.
...
function is a good exemple of measures that have to be taken in order to avoid race condition when
...
and right, in the face objects. The flux computation can then compute the fluxes and store it in a
