...
put more pressure on the limited cache of \acrshortpl{acr:GPU}, as threads will loop on different
...
\begin{figure}[H]
    ...
    {\includegraphics[width=0.229\textwidth]{Chapter_graphics_processing_units/media/structured_mesh}\label{fig:structured_mesh}}
    \qquad
    \subfloat[Unstructured mesh]
    {\includegraphics[width=0.225\textwidth]{Chapter_graphics_processing_units/media/unstructured_mesh}\label{fig:unstructured_mesh}}
...
memory accesses needed. This is the structure of arrays vs array of structures debate. It increases
...
bulk of its data, the yellow pressure solution array in this case, stays in place in dynamic memory.

\begin{figure}[H]
	\centering
	\includesvg[width=0.5\textwidth]{Chapter_graphics_processing_units/media/moving_elements}
...
wait until all operations queued on the stream are completed on the \acrshort{acr:GPU}. The results
...
execution stream, we allocate and fill a pressure array on the \acrshort{acr:CPU} as usual. We then
allocate an array on the \acrshort{acr:GPU}, and copy the data from the \acrshort{acr:CPU} array to
it. We can then perform computations on the \acrshort{acr:GPU} using that array with the
...
the \textit{new} or \textit{malloc} functions similarly to how we would normally do on the
\acrshort{acr:CPU}, but by the \acrshort{acr:GPU} inside \acrshort{acr:GPU} functions like kernels.
