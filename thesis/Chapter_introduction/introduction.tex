...
individual values at points, or a linear or quadratic function within a space subdivision, spectral
methods represent a function as a truncated series:
...
grids. They use \textit{block-structured} \acrshort{acr:AMR}, where uniformly refined meshes are
overlaid on coarse meshes. Another approach is \textit{tree-structured} \acrsshort{acr:AMR}, where
elements themselves are refined recursively. Khokhlov~\cite{Khokhlov1998} describes such an approach
that works fully in parallel. We will use a tree-structured approach, since it can easily work in
parallel by having elements refine independently.
% Something about performance?

% figure from Berger and Oliger with mesh refinement

% figure with tree refinement

Two types of refinement will be used in this work.
